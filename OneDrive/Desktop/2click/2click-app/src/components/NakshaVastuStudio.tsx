...
export const NakshaVastuStudio: React.FC<NakshaVastuStudioProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    | "naksha_plans"
    | "vastu_analyzer"
    | "material_fitting_catalog"
    | "conversations"
  >("naksha_plans");

  // Naksha State
  const [nakshaList, setNakshaList] =
    useState<HouseNakshaPlan[]>(SAMPLE_NAKSHA_PLANS);
  ...
  const [selectedPlanModal, setSelectedPlanModal] =
    useState<HouseNakshaPlan | null>(null);
  ...
  const [vastuSubTab, setVastuSubTab] = useState<
    "ai_consultant" | "direction_matrix"
  >("ai_consultant");
  const [compassDegree, setCompassDegree] = useState<number>(45);
  const [selectedPadZone, setSelectedPadZone] = useState<string | null>(
    "Ishan (North-East)",
  );
  ...
  const [aiVastuForm, setAiVastuForm] = useState({
    ...
    layoutDescription:
      "Main entrance in North-East (Ishan). Kitchen in South-East (Agneya) with cooking counter facing East. Master bedroom in South-West (Nairutya). Pooja room in North-East corner. Children bedroom in North-West. Toilet in North-West (Vayavya). Central Brahmasthan kept open and light.",
    attachedNakshaImage:
  ...
  const [vastuChatHistory, setVastuChatHistory] = useState<
  ...
  const [orderedRemedyModalItem, setOrderedRemedyModalItem] = useState<{
    name: string;
    price: string;
  } | null>(null);
  ...
  const loadVastuPreset = (
    presetKey:
      "auspicious_3bhk" | "east_2bhk" | "southwest_dosha" | "commercial_dukan",
  ) => {
    if (presetKey === "auspicious_3bhk") {
      setAiVastuForm({
        ...
        staircaseDirection: "South-West",
        waterTankPlacement:
          "North-East underground sump, South-West overhead tank",
        layoutDescription:
          "Main entrance in North-East (Ishan). Kitchen in South-East (Agneya) with cooking counter facing East. Master bedroom in South-West (Nairutya). Pooja room in North-East corner. Children bedroom in North-West. Toilet in North-West (Vayavya). Central Brahmasthan kept open and light.",
        attachedNakshaImage:
          "https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=600&q=80",
      ...
      setAiVastuForm({
        ...
        layoutDescription:
          "Main entrance in East direction. Kitchen in South-East (Agneya Kona). Living Room in North-East. Master Bedroom in South-West. Toilet in West zone. Staircase on South wall climbing clockwise.",
        attachedNakshaImage:
      ...
      setAiVastuForm({
        ...
        layoutDescription:
          "Main entrance in South-West direction. Kitchen placed in North-East corner. Toilet placed next to main entrance in South-West. Underground water tank in South-West. Staircase constructed in North-East Ishan zone. Needs zero-demolition Vastu remedies for structural stability and health.",
        attachedNakshaImage:
      ...
      setAiVastuForm({
        ...
        layoutDescription:
          "Glass storefront entrance in North (Kuber Zone). Cash counter in North-East facing East/North. Owner seating in South-West facing North. Heavy inventory storage in West/South-West. Bright warm lighting at entrance.",
        attachedNakshaImage:
  ...
  const [materials, setMaterials] = useState<MaterialWithFittingItem[]>(
    SAMPLE_MATERIALS_WITH_FITTING,
  );
  const [selectedMaterialCategory, setSelectedMaterialCategory] =
    useState<string>("All");
  const [selectedItemsForEstimate, setSelectedItemsForEstimate] = useState<
    string[]
  >([]);
  ...
  const [materialForm, setMaterialForm] = useState({
    ...
    plumberOrElectricianRole:
      "Mason" as MaterialWithFittingItem["plumberOrElectricianRole"],
  ...
  const [conversations, setConversations] =
    useState<VendorConversation[]>(SAMPLE_CONVERSATIONS);
  ...
  const activeVastuRules: VastuRoomRule[] = SAMPLE_VASTU_RULES.map((rule) => {
    ...
    const isCompliant = rule.idealDirections.some(
      (d) =>
        d.toLowerCase().includes(currentDir.toLowerCase()) ||
        currentDir.toLowerCase().includes(d.toLowerCase()),
    );
    return {
      ...rule,
  ...
  const totalVastuScore = Math.round(
    activeVastuRules.reduce((acc, r) => acc + r.score, 0) /
      activeVastuRules.length,
  );

  // Toggle Material Selection
  const toggleSelectMaterial = (id: string) => {
    if (selectedItemsForEstimate.includes(id)) {
      setSelectedItemsForEstimate(
        selectedItemsForEstimate.filter((item) => item !== id),
      );
    } else {
  ...
  const selectedMaterialsSummary = materials.filter((m) =>
    selectedItemsForEstimate.includes(m.id),
  );
  const totalMaterialPrice = selectedMaterialsSummary.reduce(
    (acc, m) => acc + m.materialPriceINR,
    0,
  );
  const totalLaborFittingPrice = selectedMaterialsSummary.reduce(
    (acc, m) => acc + m.fittingLaborChargeINR,
    0,
  );
  ...
  const handleOpenAddMaterialModal = () => {
    ...
    setMaterialForm({
      ...
      imageUrl:
        "https://images.unsplash.com/photo-1504307651254-35680f356dfd?auto=format&fit=crop&w=600&q=80",
      specs:
        "ISI certified material with high compressive strength & dampness resistance.",
  ...
  const handleOpenEditMaterialModal = (item: MaterialWithFittingItem) => {
    ...
    setMaterialForm({
      ...
      officialWebsiteUrl:
        item.officialWebsiteUrl || "https://www.ultratechcement.com",
    });
    setIsMaterialModalOpen(true);
  };
  ...
  const handleOpenLandingModal = (item: MaterialWithFittingItem) => {
    setActiveLandingModal({
      ...
      officialWebsiteUrl:
        ...
        `https://www.google.com/search?q=${encodeURIComponent(item.brandName + " " + item.itemTitle)}`,
      imageUrl: item.imageUrl,
      specs: item.specs,
      materialPriceINR: item.materialPriceINR,
      fittingLaborChargeINR: item.fittingLaborChargeINR,
  ...
  const handleStartConversationWithVendor = (
    vendorName: string,
    role:
      | "Architect"
      | "Plumber"
  ...
  const filteredNakshas = nakshaList.filter((plan) => {
    const matchesFacing =
      facingFilter === "All" || plan.facingDirection === facingFilter;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-teal-950 to-indigo-950 text-white p-6 sm:p-8 rounded-3xl shadow-xl border border-teal-800/80 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-2 max-w-2xl">
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">
            Ghar Banawao &amp; Renovation Hub (Naksha, Vastu &amp; Material
            Fitting Rates)
          </h1>
          <p className="text-xs sm:text-sm text-teal-100/90 leading-relaxed">
            Choose 2D/3D Naksha Blueprints, check Vastu score, pick
            Plumber/Electrician fittings with brand model prices &amp; labor
            charges, and converse directly with Architects, Plumbers,
            Electricians, Dukandars &amp; Contractors!
          </p>
        </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 dark:border-slate-800 pb-2 overflow-x-auto scrollbar-none">
        ...
        >
          <MessageSquare className="w-4 h-4" />
          <span>
            Architect, Plumber &amp; Dukandar Chat ({conversations.length})
          </span>
        </button>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-3 flex-wrap text-xs">
              ...
              {(["All", "North", "East", "South", "West"] as const).map(
                (dir) => (
                  <button
                    key={dir}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredNakshas.map((plan) => (
              <div
                key={plan.id}
                className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm hover:shadow-md transition space-y-4 flex flex-col justify-between"
              >
                <div>
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-xs text-teal-600 dark:text-teal-400 font-semibold mt-0.5">
                          {plan.architectName} • Rating: ★{" "}
                          {plan.architectRating}
                        </p>
                      </div>
                ...
                <div className="p-5 pt-0 border-t border-slate-100 dark:border-slate-700/80 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 text-xs">
                  <div>
                    ...
                    <span className="text-base font-extrabold text-teal-700 dark:text-teal-300">
                      ₹{(plan.estimatedConstructionCostINR / 100000).toFixed(2)}{" "}
                      Lakhs
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() =>
                        alert(
                          `Downloaded 2D CAD Blueprint & Vastu Certificate for ${plan.title}!`,
                        )
                      }
                    ...
                    <button
                      onClick={() =>
                        handleStartConversationWithVendor(
                          plan.architectName,
                          "Architect",
                          `Customization of ${plan.title}`,
                        )
                      }
          ...
          <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span>
                  AI Vastu Shastra Consultant &amp; Architectural Report
                </span>
              </button>
          ...
          {vastuSubTab === "ai_consultant" && (
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-slate-900 via-amber-950 to-slate-900 text-white p-6 rounded-2xl border border-amber-800/80 shadow-lg space-y-4">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/20 text-amber-300 text-xs font-extrabold border border-amber-500/30 mb-2">
                      ...
                      <span>
                        AI Vastu Shastra Consultant &amp; Architectural Analyst
                      </span>
                    </div>
                    <h2 className="text-xl font-black text-white">
                      Naksha Vastu Analysis &amp; Non-Demolition Remedial Report
                      Generator
                    </h2>
                    <p className="text-xs text-amber-100/90 mt-1 max-w-2xl">
                      Analyze layout descriptions, property dimensions, and
                      floor plans across North-East (Ishan), South-East
                      (Agneya), South-West (Nairutya), North-West (Vayavya), and
                      Brahmasthan zones.
                    </p>
                  </div>
                ...
                <div className="pt-3 border-t border-amber-800/60 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs">
                    ...
                    >
                      <span>
                        ⚠️ 40x60 South-West Facing (Agni/Nairutya Dosha)
                      </span>
                    </button>
                    <button
                      type="button"
              ...
              <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs space-y-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-4 border-b border-slate-100 dark:border-slate-700">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Input digital compass angle, 9x9 Pad Vinyas layout, and
                      room arrangements for AI Vastu Analysis
                    </p>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="text-sm font-black text-white flex items-center gap-2">
                        ...
                        <span className="px-2 py-0.5 rounded bg-amber-500/30 text-amber-200 text-[10px] font-bold">
                          {compassDegree >= 22.5 && compassDegree < 67.5
                            ? "North-East (Ishan)"
                            : compassDegree >= 67.5 && compassDegree < 112.5
                              ? "East (Aditya)"
                              : compassDegree >= 112.5 && compassDegree < 157.5
                                ? "South-East (Agneya)"
                                : compassDegree >= 157.5 &&
                                    compassDegree < 202.5
                                  ? "South (Yama)"
                ...
                <form
                  onSubmit={handleGenerateAiVastuReport}
                  className="space-y-6"
                >
                  ...
                  <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 space-y-2">
                    <div className="flex justify-between items-center text-xs font-bold text-slate-800 dark:text-slate-200">
                      <span className="flex items-center gap-1.5">
                        <Compass className="w-4 h-4 text-amber-500" />
                        Set Main Entrance Compass Facing Orientation (
                        {compassDegree}°):
                      </span>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                        ...
                        Interactive 9x9 Pad Vinyas Grid &amp; Elemental Zone
                        Selector:
                      </span>
                      {selectedPadZone && (
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center text-xs font-bold">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setSelectedPadZone("Kuber (North - Wealth)")
                        }
                        className={`p-3 rounded-xl border transition flex flex-col items-center justify-center gap-1 ${
                          selectedPadZone?.includes("North -")
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setSelectedPadZone("Ishan (North-East - Water)")
                        }
                        className={`p-3 rounded-xl border transition flex flex-col items-center justify-center gap-1 ${
                          selectedPadZone?.includes("North-East")
                            ? "bg-amber-500 text-slate-950 border-amber-600 shadow-md ring-2 ring-amber-300 font-black"
                            : "bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:bg-slate-100"
                        }`}
                      >
                        <span className="text-[10px] uppercase font-black opacity-80">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setSelectedPadZone("Varun (West - Stability)")
                        }
                        className={`p-3 rounded-xl border transition flex flex-col items-center justify-center gap-1 ${
                          selectedPadZone?.includes("West -")
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setSelectedPadZone("Brahmasthan (Center - Space)")
                        }
                        className={`p-3 rounded-xl border transition flex flex-col items-center justify-center gap-1 ${
                          selectedPadZone?.includes("Brahmasthan")
                            ? "bg-gradient-to-r from-amber-500 to-orange-500 text-slate-950 border-orange-600 shadow-md ring-2 ring-amber-300 font-black"
                      ...
                      >
                        <span className="text-[10px] uppercase font-black opacity-90">
                          🏛️ Brahmasthan
                        </span>
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setSelectedPadZone("Aditya (East - Light)")
                        }
                        className={`p-3 rounded-xl border transition flex flex-col items-center justify-center gap-1 ${
                          selectedPadZone?.includes("East -")
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setSelectedPadZone("Nairutya (South-West - Earth)")
                        }
                        className={`p-3 rounded-xl border transition flex flex-col items-center justify-center gap-1 ${
                          selectedPadZone?.includes("South-West")
                            ? "bg-stone-700 text-white border-stone-800 shadow-md ring-2 ring-stone-400"
                            : "bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:bg-slate-100"
                        }`}
                      >
                        <span className="text-[10px] uppercase font-black opacity-80">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setSelectedPadZone("Yama (South - Health)")
                        }
                        className={`p-3 rounded-xl border transition flex flex-col items-center justify-center gap-1 ${
                          selectedPadZone?.includes("South -")
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setSelectedPadZone("Agneya (South-East - Fire)")
                        }
                        className={`p-3 rounded-xl border transition flex flex-col items-center justify-center gap-1 ${
                          selectedPadZone?.includes("South-East")
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                    <div>
                      ...
                      >
                        <option value="3BHK Residential House">
                          3BHK Residential House
                        </option>
                        <option value="2BHK Independent House">
                          2BHK Independent House
                        </option>
                        <option value="4BHK Duplex Villa">
                          4BHK Duplex Villa
                        </option>
                        <option value="Commercial Shop & Showroom">
                          Commercial Shop &amp; Showroom
                        </option>
                        <option value="Plot / Land Naksha">
                          Plot / Land Naksha
                        </option>
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setAiVastuForm({
                            ...aiVastuForm,
                            mainEntranceDirection: e.target.value,
                          })
                        }
                        className="w-full p-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-slate-800 dark:text-slate-200 focus:outline-none"
                      >
                        <option value="North-East">
                          North-East (Ishan - Highly Auspicious)
                        </option>
                        <option value="East">
                          East (Auspicious Surya Gate)
                        </option>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setAiVastuForm({
                            ...aiVastuForm,
                            staircaseDirection: e.target.value,
                          })
                        }
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="font-bold text-slate-800 dark:text-slate-200 block">
                          Attached Floor Plan (Naksha Blueprint)
                        </span>
                        <span className="text-[11px] text-slate-500">
                          Auto-scanned image / CAD draft ready for AI
                          directional overlay
                        </span>
                      </div>
                    </div>
                          ...
                          if (file) {
                            ...
                            reader.onloadend = () => {
                              if (reader.result) {
                                setAiVastuForm({
                                  ...aiVastuForm,
                                  attachedNakshaImage: reader.result as string,
                                });
                              }
                            };
                            reader.readAsDataURL(file);
              ...
              {vastuReportOutput && (
                <div className="space-y-6">
                    ...
                    <div className="bg-gradient-to-r from-slate-900 via-amber-950 to-slate-900 text-white p-6 border-b border-amber-800/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-300">
                            • 2click.in Naksha &amp; Vastu Studio
                          </span>
                        </div>
                        <h3 className="text-lg font-black text-white">
                          Vastu Audit Report for{" "}
                          {vastuReportOutput.propertyType}
                        </h3>
                      </div>
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        >
                          ...
                          <span>
                            {copyReportSuccess ? "Copied!" : "Copy Text"}
                          </span>
                        </button>
                    ...
                    <div className="p-6 sm:p-8 space-y-6 text-slate-800 dark:text-slate-200">
                      ...
                      <div className="pt-6 border-t border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs bg-amber-50/50 dark:bg-amber-950/30 p-4 rounded-2xl border border-amber-200/50 dark:border-amber-800/50">
                        <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300 font-semibold">
                          ...
                          <span>
                            Verified Zero-Demolition Vastu Remedies based on
                            Vedic Pancha Tattva Principles
                          </span>
                        </div>
                        <button
                          onClick={() =>
                  ...
                  {vastuReportOutput.remedies &&
                    vastuReportOutput.remedies.length > 0 && (
                      <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-md space-y-4">
                        <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-700 pb-3">
                          <div>
                            <h4 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                              ...
                              Non-Destructive Vastu Remedies on 2click.in
                              Marketplace
                            </h4>
                            <p className="text-xs text-slate-500">
                              Order verified energetic balancing items directly
                              from multi-vendor stores
                            </p>
                          </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-xs">
                          {vastuReportOutput.remedies.map((remedy, idx) => (
                            <div
                              key={idx}
                              className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 space-y-3 hover:border-amber-400 transition flex flex-col justify-between"
                            >
                              <div className="space-y-1">
                                ...
                                <p className="text-[11px] text-slate-500">
                                  Zero-Demolition energetic balance for home
                                  &amp; office
                                </p>
                              </div>
                              ...
                              <div className="pt-2 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center">
                                ...
                                <button
                                  onClick={() =>
                                    setOrderedRemedyModalItem(remedy)
                                  }
                  ...
                  <div className="bg-slate-900 text-white p-6 rounded-3xl border border-amber-800/80 shadow-xl space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2 text-[11px] pt-1">
                      <button
                        ...
                        onClick={() =>
                          handleSendVastuChat(
                            "What are the best wall colors for my kitchen and bedroom?",
                          )
                        }
                      ...
                      <button
                        ...
                        onClick={() =>
                          handleSendVastuChat(
                            "Where should I place mirror and dressing table?",
                          )
                        }
                        className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-amber-300 rounded-lg border border-slate-700 transition"
                      >
                      ...
                      <button
                        ...
                        onClick={() =>
                          handleSendVastuChat(
                            "Where should study table and work desk face?",
                          )
                        }
                        className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-amber-300 rounded-lg border border-slate-700 transition"
                      >
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onKeyDown={(e) =>
                          e.key === "Enter" && handleSendVastuChat()
                        }
                ...
                <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4 z-50">
                  <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl max-w-md w-full border border-amber-400 shadow-2xl space-y-4 text-xs">
                    ...
                    <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 space-y-2">
                      <span className="text-[10px] uppercase font-black text-amber-600">
                        Selected Remedy Item
                      </span>
                      <div className="text-sm font-bold text-slate-900 dark:text-white">
                        {orderedRemedyModalItem.name}
                      </div>
                      <div className="text-base font-black text-amber-600 dark:text-amber-400">
                        {orderedRemedyModalItem.price}
                      </div>
                    </div>

                    <p className="text-slate-600 dark:text-slate-300">
                      This non-destructive Vastu remedy will be dispatched
                      directly to your doorstep from verified 2click.in Vastu
                      suppliers with installation instructions.
                    </p>
                    ...
                    <div className="flex gap-2 justify-end pt-2">
                      ...
                      <button
                        onClick={() => {
                          alert(
                            `Order placed successfully for ${orderedRemedyModalItem.name}! Supplier notification sent.`,
                          );
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs space-y-6">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-100 dark:border-slate-700 pb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Select your home's room placements to instantly analyze
                    overall Vastu alignment &amp; corrective remedies
                  </p>
                </div>
                ...
                <div className="p-4 rounded-2xl bg-gradient-to-r from-slate-900 to-teal-950 text-white flex items-center gap-4 shadow-md border border-teal-800">
                  ...
                  <div>
                    <div className="text-xs font-bold text-teal-300">
                      {totalVastuScore >= 85
                        ? "Highly Auspicious Vastu"
                        : totalVastuScore >= 65
                          ? "Moderate Vastu (Remedies Needed)"
                          : "Flawed Vastu Alignment"}
                    </div>
                    <div className="text-[10px] text-slate-300">
                      Based on Vedic Architectural Manuals
                    </div>
                  </div>
                </div>
              </div>
                ...
                {activeVastuRules.map((rule) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        <option value="North-East">
                          North-East (Ishan Kona)
                        </option>
                        <option value="South-East">
                          South-East (Agni Kona)
                        </option>
                        <option value="South-West">
                          South-West (Nairutya Kona)
                        </option>
                        <option value="North-West">
                          North-West (Vayu Kona)
                        </option>
                    ...
                    <p className="text-[11px] text-slate-600 dark:text-slate-400">
                      <span className="font-bold">Ideal Directions:</span>{" "}
                      {rule.idealDirections.join(", ")}
                    </p>
              ...
              <div className="p-4 rounded-xl bg-slate-100 dark:bg-slate-900 flex justify-between items-center text-xs">
                <span className="text-slate-700 dark:text-slate-300 font-semibold">
                  Need official Vastu approval certificate for your bank loan or
                  builder sanction?
                </span>
                <button
                  onClick={() =>
                    handleStartConversationWithVendor(
                      "Ar. Rajesh Sharma (Senior Architect)",
                      "Architect",
                      "Official Vastu Inspection Certificate",
                    )
                  }
          ...
          <div className="bg-gradient-to-r from-slate-900 via-teal-950 to-slate-900 text-white p-6 rounded-2xl border border-teal-800/80 shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <h2 className="text-lg font-bold flex items-center gap-2">
                Construction &amp; Renovation Materials + Fitting Charges
                Catalog
              </h2>
              <p className="text-xs text-slate-300 mt-1">
                Add new materials with photos, specs &amp; official website
                links or edit existing material catalog entries.
              </p>
            </div>
                ...
                <div className="bg-teal-900/80 p-3 rounded-xl border border-teal-700 text-xs flex items-center gap-4 shrink-0">
                  <div>
                    <span className="text-[10px] text-teal-300 font-bold block">
                      {selectedItemsForEstimate.length} Items Selected
                    </span>
                    <span className="text-sm font-extrabold text-white">
                      ₹
                      {(
                        totalMaterialPrice + totalLaborFittingPrice
                  ...
                  <div className="text-[10px] text-slate-300 border-l border-teal-700 pl-3">
                    <div>
                      Material: ₹{totalMaterialPrice.toLocaleString("en-IN")}
                    </div>
                    <div>
                      Fitting Labor: ₹
                      {totalLaborFittingPrice.toLocaleString("en-IN")}
                    </div>
                  </div>
                </div>
              )}
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <div
                      className="relative h-52 bg-slate-900 overflow-hidden group cursor-pointer"
                    ...
                    >
                      ...
                      <div className="absolute top-3 right-3 bg-slate-950/85 backdrop-blur-md px-2.5 py-1 rounded-xl text-[10px] font-extrabold text-amber-300 border border-amber-500/30 flex items-center gap-1">
                        {item.plumberOrElectricianRole === "Plumber" && (
                          <Droplets className="w-3 h-3 text-cyan-400" />
                        )}
                        {item.plumberOrElectricianRole === "Electrician" && (
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center p-4 text-center">
                        <span className="px-3 py-1.5 bg-amber-500 text-slate-950 font-extrabold rounded-xl text-xs flex items-center gap-1.5 shadow-lg">
                          <Globe className="w-4 h-4" /> Visit Official Web
                          Landing Page →
                        </span>
                      </div>
                    </div>
                      ...
                      <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 space-y-2 text-xs">
                        <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-2">
                          ...
                          <span className="font-extrabold text-slate-900 dark:text-white text-sm">
                            ₹{item.materialPriceINR.toLocaleString("en-IN")}{" "}
                        ...
                        <div className="flex justify-between items-center text-teal-700 dark:text-teal-300">
                          <span className="font-semibold flex items-center gap-1">
                            ...
                            {item.plumberOrElectricianRole} Labor Fitting
                            Charge:
                          </span>
                          <span className="font-extrabold">
                            + ₹
                            {item.fittingLaborChargeINR.toLocaleString("en-IN")}{" "}
                        ...
                        <div className="pt-2 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center font-black text-amber-600 dark:text-amber-400 text-sm">
                          ...
                          <span>
                            ₹
                            {(
                              item.materialPriceINR + item.fittingLaborChargeINR
                            ).toLocaleString("en-IN")}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1.5 pt-1 text-[11px]">
                        ...
                        >
                          <span className="flex items-center gap-1.5 text-[10px] text-teal-600 dark:text-teal-400">
                            <Globe className="w-3.5 h-3.5" /> Official Brand
                            Landing Page
                          </span>
                  ...
                  <div className="p-4 pt-0 flex items-center justify-between gap-2 text-xs">
                    ...
                    >
                      ...
                      <span>
                        {isSelected
                          ? "Selected in Renovation"
                          : "Add to Fitting Rate List"}
                      </span>
                    </button>
          ...
          <div className="lg:col-span-8 flex flex-col justify-between p-6 bg-white dark:bg-slate-800 space-y-4">
            ...
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-700 pb-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() =>
                    alert(
                      `Calling ${activeConv.vendorName} at ${activeConv.vendorPhone}...`,
                    )
                  }
            ...
            <div className="flex-1 space-y-4 overflow-y-auto max-h-[380px] p-2 pr-4">
              {activeConv.messages.map((msg) => {
                const isMe =
                  msg.senderId === (currentUser ? currentUser.id : "user");
                return (
                  <div
                    key={msg.id}
                      ...
                      {msg.attachments && msg.attachments.length > 0 && (
                        <div className="mt-2.5 pt-2 border-t border-white/20 dark:border-slate-600 space-y-1">
                          {msg.attachments.map((att, idx) => (
                            <div
                              key={idx}
                              className="p-2 rounded-xl bg-black/20 text-[11px] font-bold flex items-center justify-between gap-2"
                            >
                              ...
                              <button
                                onClick={() =>
                                  alert(`Opening attachment ${att.title}...`)
                                }
            ...
            <form
              ...
              className="pt-3 border-t border-slate-100 dark:border-slate-700 flex gap-2"
            >
              <input
                type="text"
      ...
      {isMaterialModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl relative space-y-6 max-h-[90vh] overflow-y-auto text-slate-900 dark:text-white">
            <div className="flex justify-between items-start border-b border-slate-200 dark:border-slate-800 pb-4">
              <div>
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-teal-500/10 text-teal-600 dark:text-teal-400 font-bold text-xs mb-1">
                  ...
                  {editingMaterialId
                    ? "Edit Material Details & Photo"
                    : "Add New Material with Photo & Specs"}
                </div>
                <h2 className="text-xl font-black">
                  {editingMaterialId
                    ? "Update Material Catalog Item"
                    : "New Construction Material Entry"}
                </h2>
                <p className="text-xs text-slate-500">
                  Provide photo, specs, selling price, fitting labor charges and
                  official website landing page URL.
                </p>
              </div>
              <button
            ...
            <form
              onSubmit={handleSaveMaterialSubmit}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setMaterialForm({
                        ...materialForm,
                        materialPriceINR: Number(e.target.value),
                      })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setMaterialForm({
                        ...materialForm,
                        fittingLaborChargeINR: Number(e.target.value),
                      })
                    }
              ...
              <div className="space-y-3 p-4 bg-amber-50/50 dark:bg-amber-950/20 rounded-2xl border border-amber-200/60 dark:border-amber-800/40">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[10px] text-amber-700 dark:text-amber-400">
                    Select file or click preset photo
                  </span>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 mt-1">
                    Clicking material card or brand web page will land directly
                    on this official website page smoothly!
                  </p>
                </div>
              </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setMaterialForm({
                        ...materialForm,
                        empanelledSupplier: e.target.value,
                      })
                    }
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setMaterialForm({
                        ...materialForm,
                        plumberOrElectricianRole: e.target.value as any,
                      })
                    }
                    className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-bold"
                  >
                    <option value="Mason">
                      Mason (Cement / Bricks / Tiles)
                    </option>
                    <option value="Plumber">
                      Plumber (Pipes &amp; Bathroom Fittings)
                    </option>
                    <option value="Electrician">
                      Electrician (Wires &amp; Switches)
                    </option>
                    <option value="Painter">
                      Painter (Paints &amp; Putty)
                    </option>
                    <option value="Carpenter">
                      Carpenter (Cabinets &amp; Doors)
                    </option>
                    <option value="Sub-Contractor">
                      Sub-Contractor (Railings &amp; Glass)
                    </option>
                    <option value="General Vendor">
                      General Vendor / Dukandar
                    </option>
                  </select>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="px-2 py-0.5 rounded bg-teal-500/20 text-teal-300 text-[10px] font-extrabold border border-teal-500/30 flex items-center gap-1">
                    <Globe className="w-3 h-3 text-teal-400" /> Official
                    Manufacturer Landing Page
                  </span>
                  <span className="text-[10px] text-slate-400 font-mono font-bold">
                    {activeLandingModal.modelNumber}
                  </span>
                </div>
                <h2 className="text-sm font-black text-white">
                  {activeLandingModal.itemTitle} ({activeLandingModal.brandName}
                  )
                </h2>
              </div>
            </div>
            ...
            <div className="bg-slate-100 dark:bg-slate-950 px-4 py-2 border-b border-slate-200 dark:border-slate-800 flex items-center gap-3 text-xs">
              ...
              <div className="flex-1 bg-white dark:bg-slate-800 px-3 py-1 rounded-lg border border-slate-200 dark:border-slate-700 font-mono text-[11px] text-slate-600 dark:text-slate-300 flex items-center gap-2">
                ...
                <span className="truncate">
                  {activeLandingModal.officialWebsiteUrl}
                </span>
              </div>
            </div>
              ...
              <div className="lg:col-span-4 bg-slate-900 text-white p-6 border-l border-slate-800 overflow-y-auto space-y-5 flex flex-col justify-between">
                <div className="space-y-4">
                  <div className="relative h-44 rounded-2xl overflow-hidden border border-slate-700 shadow-md">
                    <img
                      src={activeLandingModal.imageUrl}
                      alt={activeLandingModal.itemTitle}
                      referrerPolicy="no-referrer"
                      className="w-full h-full object-cover"
                    />
                  ...
                  <div>
                    <h3 className="font-extrabold text-base text-white">
                      {activeLandingModal.itemTitle}
                    </h3>
                    <p className="text-xs text-slate-400 mt-1">
                      {activeLandingModal.specs}
                    </p>
                  </div>
                  ...
                  <div className="p-4 rounded-xl bg-slate-800/80 border border-slate-700 space-y-2 text-xs">
                    <div className="flex justify-between items-center text-slate-300">
                      ...
                      <span className="font-extrabold text-emerald-400 font-mono text-sm">
                        ₹
                        {activeLandingModal.materialPriceINR.toLocaleString(
                          "en-IN",
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-slate-300">
                      ...
                      <span className="font-extrabold text-amber-400 font-mono">
                        + ₹
                        {activeLandingModal.fittingLaborChargeINR.toLocaleString(
                          "en-IN",
                        )}
                      </span>
                    </div>
                    <div className="pt-2 border-t border-slate-700 flex justify-between items-center font-black text-teal-300 text-sm">
                      ...
                      <span>
                        ₹
                        {(
                          activeLandingModal.materialPriceINR +
                          activeLandingModal.fittingLaborChargeINR
                        ).toLocaleString("en-IN")}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-3 bg-teal-950/60 border border-teal-800 rounded-xl text-[11px] text-teal-200 space-y-1">
                    <p className="font-bold">
                      ✓ Smooth Returning Navigation Active:
                    </p>
                    <p>
                      You can browse the manufacturer page or click "← Back to
                      Build Platform" above to return without losing any data!
                    </p>
                  </div>
                </div>
