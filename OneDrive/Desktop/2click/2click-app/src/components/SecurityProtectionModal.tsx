...
export const SecurityProtectionModal: React.FC<
  SecurityProtectionModalProps
> = ({ isOpen, onClose, currentUserRole = "Customer" }) => {
  const [activeTab, setActiveTab] = useState<
    "overview" | "scanner" | "privacy" | "audit"
  >("overview");
  ...
  const handleSanitizeCache = () => {
    ...
    setSanitizedNotice(
      "Security Cache & Temporary Storage Successfully Sanitized!",
    );
      ...
      <div className="relative w-full max-w-4xl bg-slate-900 border border-teal-500/30 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-bold text-white tracking-wide">
                  2click.in Website Security & Protection
                </h2>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Bank-Grade 256-Bit SSL Encryption, Firewalls, Anti-DDoS,
                Firebase Security Rules & DPDP Act 2023 Security
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6 bg-slate-900/90 text-slate-200">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-base font-semibold text-white">
                      Website Protection Index: 99/100
                    </h3>
                    <p className="text-xs text-slate-400">
                      All 6 Core Defense Engines Operating at Peak Security
                    </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-xl space-y-2">
                  ...
                  <h4 className="text-sm font-semibold text-white">
                    SSL 256-Bit Transport
                  </h4>
                  <p className="text-xs text-slate-400">
                    HTTPS TLS 1.3 RSA/ECC encryption with Strict Transport
                ...
                <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-xl space-y-2">
                  ...
                  <p className="text-xs text-slate-400">
                    Cloud SQL PostgreSQL queries parameterized with Drizzle ORM
                    to block all injection vectors.
                  </p>
                </div>
                ...
                <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-xl space-y-2">
                  ...
                  <p className="text-xs text-slate-400">
                    Firebase Admin JWT token verification and Firestore Security
                    Rules isolate tenant data.
                  </p>
                </div>
                ...
                <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-xl space-y-2">
                  ...
                  <p className="text-xs text-slate-400">
                    Throttles API requests to 120 req/min per IP to eliminate
                    bot attacks & brute force attempts.
                  </p>
                </div>
                ...
                <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-xl space-y-2">
                  ...
                  <h4 className="text-sm font-semibold text-white">
                    Anti-XSS & CSP Headers
                  </h4>
                  <p className="text-xs text-slate-400">
                    HTTP response headers (Nosniff, SameOrigin, XSS-Filter) stop
                ...
                <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-xl space-y-2">
                  ...
                  <p className="text-xs text-slate-400">
                    Full compliance with Indian Digital Personal Data Protection
                    Act 2023 and GDPR guidelines.
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-950/60 border border-slate-800 rounded-xl flex items-center justify-between">
                <div>
                  <h4 className="text-xs font-semibold text-white">
                    Session & Cache Sanitization
                  </h4>
                  <p className="text-[11px] text-slate-400">
                    Purge stale temporary cached data from local browser
                    storage.
                  </p>
                </div>
                <button
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Simulate real-time security penetration test across all
                    endpoints.
                  </p>
                </div>
                <button
              ...
              <div className="p-4 bg-black border border-slate-800 rounded-xl font-mono text-xs text-teal-400 min-h-[220px] max-h-[300px] overflow-y-auto space-y-2">
                {scanLogs.length === 0 ? (
                  <p className="text-slate-600 italic">
                    Click "Start Full Audit Scan" to launch live vulnerability
                    analysis...
                  </p>
                ) : (
                  scanLogs.map((log, idx) => (
                    <p key={idx} className="leading-relaxed animate-fade-in">
                      {log}
                    </p>
                  ))
                )}
            ...
            <div className="space-y-5">
              <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-xl space-y-3">
                ...
                <p className="text-xs text-slate-300 leading-relaxed">
                  2click.in guarantees strict data privacy compliance for all
                  Indian customers, contractors, architects, and financial
                  partners.
                </p>
                <ul className="space-y-2 text-xs text-slate-400">
                  <li className="flex items-center gap-2">
                    ...
                    <span>
                      <strong>Consent-Based Processing:</strong> User data is
                      collected solely for construction BOQ, loan processing,
                  ...
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                    <span>
                      <strong>Zero Data Selling Guarantee:</strong> Your phone
                      number, email, project specs, and BOQ estimates are never
                      sold or shared with external ad networks.
                    </span>
                  </li>
                  <li className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                    <span>
                      <strong>Right to Erasure:</strong> Request instant account
                      & document wipe directly from user profile settings.
                    </span>
                  </li>
                </ul>
              </div>
              ...
              <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-xl flex items-center justify-between">
                <div>
                  <h4 className="text-xs font-semibold text-white">
                    Strict IP-Session Binding
                  </h4>
                  <p className="text-[11px] text-slate-400">
                    Automatically invalidate user authentication token if IP
                    address abruptly changes.
                  </p>
                </div>
          ...
          {activeTab === "audit" && (
            <div className="space-y-5">
              <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-xl space-y-3">
                <h3 className="text-sm font-bold text-white">
                  Official Security & Compliance Verification
                </h3>
                <p className="text-xs text-slate-400">
                  Download an officially signed security summary report
                  detailing 2click.in’s current encryption, SSL grade, and
                  database defense specs.
                </p>
                <div className="pt-2">
                  <button
                  ...
                  >
                    <Download className="w-4 h-4" /> Download Security
                    Compliance Report
                  </button>
                </div>
              </div>
        ...
        <div className="p-4 bg-slate-950 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
          <div className="flex items-center gap-2">
            ...
            <span>
              2click Security Engine Active • User Role:{" "}
              <strong className="text-slate-200">{currentUserRole}</strong>
            </span>
          </div>
          <button
            onClick={onClose}
