...
export const PERMISSION_MODULE_DEFINITIONS = [
  {
    key: "canAccessCivilBoq",
    label: "Civil BOQ & Material Estimator",
    category: "Engineering",
    icon: Building2,
    desc: "Cement, steel, brick & aggregate calculation studio",
  },
  {
    key: "canAccessSolarEngine",
    label: "Solar Rooftop & Subsidy Calculator",
    category: "Renewable",
    icon: Sun,
    desc: "Rooftop solar panel, battery & Govt subsidy engine",
  },
  {
    key: "canAccessDukandarMarket",
    label: "Dukandar B2B Shopkeeper Portal",
    category: "Commerce",
    icon: Store,
    desc: "Wholesale product catalog, inventory & customer orders",
  },
  {
    key: "canAccessLogistics",
    label: "Logistics, Fleet & E-Way Freight Hub",
    category: "Transport",
    icon: Truck,
    desc: "Bulk tippers, Tata Ace, crane trailers, freight calculator & E-Way Tracking",
  },
  {
    key: "canAccessBankLoans",
    label: "Bank Loans & Credit Sanction Hub",
    category: "Finance",
    icon: Landmark,
    desc: "Loan applications, interest rate comparison & credit verification",
  },
  {
    key: "canAccessWaterEtpStp",
    label: "Water Treatment & ETP/STP Studio",
    category: "Engineering",
    icon: Droplets,
    desc: "Sewage plant design, flow rates & chemical dosage",
  },
  {
    key: "canAccessElectricalMep",
    label: "Electrical MEP & Transformer Studio",
    category: "Engineering",
    icon: Zap,
    desc: "Load calculations, single line diagram & panel sizing",
  },
  {
    key: "canAccessVendorBidding",
    label: "Vendor Bidding & Escrow Procurement",
    category: "Procurement",
    icon: Gavel,
    desc: "Tender submissions, competitive bids & escrow deposits",
  },
  {
    key: "canAccessLidarSurveys",
    label: "LiDAR 3D Point Cloud Survey",
    category: "Tech",
    icon: Box,
    desc: "Drone elevation data, contours & site survey maps",
  },
  {
    key: "canAccessVrTour",
    label: "VR 360 Degree Walkthrough Studio",
    category: "Tech",
    icon: Eye,
    desc: "3D spatial design walkthrough & client presentation",
  },
  {
    key: "canAccessNakshaVastu",
    label: "2D Naksha Blueprint & Vastu Engine",
    category: "Architecture",
    icon: Compass,
    desc: "House plan floor layouts & Vastu compass analysis",
  },
  {
    key: "canAccessKhatabookCrm",
    label: "Khatabook Ledger & B2B CRM",
    category: "Business",
    icon: BookOpen,
    desc: "Customer balance register, GST invoices & payment reminders",
  },
  {
    key: "canEditProductPrices",
    label: "Edit Product Rates & Wholesale Offers",
    category: "Admin Control",
    icon: Tag,
    desc: "Ability to change material price tags & discount banners",
  },
  {
    key: "canDownloadPdfReports",
    label: "Download Official PDF Reports & BOQs",
    category: "Reports",
    icon: FileText,
    desc: "Exporting branded PDFs for bank loans & government tenders",
  },
];
...
export const UserPermissionModal: React.FC<UserPermissionModalProps> = ({
  ...
  const applyPreset = (
    presetType: "all_access" | "dukandar" | "contractor" | "minimal",
  ) => {
    if (presetType === "all_access") {
      const full: UserPermissions = {};
      PERMISSION_MODULE_DEFINITIONS.forEach((m) => {
        full[m.key] = true;
      });
      setPermissions(full);
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-4xl rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden my-auto animate-in fade-in zoom-in duration-200">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h2 className="text-xl sm:text-2xl font-black text-white mt-1">
                  {user.name} - कस्टमाइज्ड डैशबोर्ड परमिशन (User Permission
                  Matrix)
                </h2>
                <p className="text-xs text-slate-300">
                  {user.email} • ID: {user.id} • District:{" "}
                  {user.district || user.city || "National"}
                </p>
              </div>
            </div>
            ...
            <div className="bg-white/10 p-3 rounded-2xl border border-white/20 text-right shrink-0">
              <span className="text-[10px] text-indigo-200 uppercase font-black block">
                Active Modules Granted
              </span>
              <span className="text-xl font-black text-emerald-400">
                {activeCount} / {PERMISSION_MODULE_DEFINITIONS.length}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-400 font-bold">
                Click tile to Toggle Permission
              </span>
            </div>
        ...
        <div className="p-4 bg-slate-100 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
          <div className="text-slate-500 font-semibold text-center sm:text-left">
            💡 परमिशन सेव करने पर उपयोगकर्ता के अकाउंट में केवल चुने गए
            मोड्यूल्स ही खुलेंगे।
          </div>
