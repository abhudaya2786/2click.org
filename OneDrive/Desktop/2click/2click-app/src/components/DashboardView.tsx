...
import {
  PERMISSION_MODULE_DEFINITIONS,
  getDefaultPermissionsForRole,
} from "./UserPermissionModal";
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-900/95 border border-slate-700 p-3 rounded-xl shadow-xl text-xs text-slate-100 backdrop-blur-md">
        <p className="font-bold border-b border-slate-700 pb-1 mb-2 text-teal-400">
          {label}
        </p>
          ...
          {payload.map((entry: any, index: number) => (
            ...
            >
              ...
              <span className="font-mono font-bold text-white">
                ₹{Number(entry.value).toFixed(1)} Lakhs
              </span>
            </div>
          ))}
        </div>
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const [shareSettings, setShareSettings] = useState<UserShareSettings>(() => {
    return (
      activeUser.shareSettings || {
        ...
        showGstin: true,
        headlineMessage:
          "अल्ट्राटेक सीमेंट एवं टाटा स्टील अधिकृत डीलर - विशेष डिस्काउंट हेतु संपर्क करें",
      }
    );
  });
  ...
  >([
    {
      ...
      inStock: true,
      description:
        "High performance cement for slab casting & foundation work.",
    },
    {
      id: "P-2",
    ...
    },
    {
      ...
      inStock: true,
      description:
        "Standard kiln burned 1st class red clay construction bricks.",
    },
  ]);

  // Public Preview Modal State
  const [showPublicPreviewModal, setShowPublicPreviewModal] =
    useState<boolean>(false);
  ...
  const [newCategory, setNewCategory] = useState<
    "construction" | "solar" | "interior" | "lidar"
  >("construction");
  ...
  const chartData =
    timeRange === "6m" ? ALL_COST_TREND_DATA.slice(-6) : ALL_COST_TREND_DATA;
  ...
  const monthOverMonthChange =
    ((lastMonthSpend - prevMonthSpend) / prevMonthSpend) * 100;
      ...
      <div className="bg-gradient-to-r from-teal-950 via-slate-900 to-indigo-950 rounded-3xl p-6 sm:p-8 text-white shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div className="flex items-start sm:items-center gap-4">
            ...
            <div className="space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {activeUser.isKycVerified && (
                  <span className="px-2 py-0.5 bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 rounded-lg text-[10px] font-extrabold flex items-center gap-1">
                    <BadgeCheck className="w-3 h-3 text-emerald-400" /> KYC
                    Verified
                  </span>
                )}
              </div>
              ...
              <p className="text-xs text-slate-300 flex items-center gap-3">
                <span>
                  📍 {activeUser.district || activeUser.city || "India"}
                </span>
                <span>•</span>
          ...
          <div className="bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/20 space-y-2 shrink-0 max-w-md">
            <span className="text-[11px] font-black text-amber-300 uppercase block flex items-center gap-1">
              <Share2 className="w-3.5 h-3.5" /> आपका शेयर करने योग्य डिजिटल
              प्रोफाइल/स्टोर लिंक
            </span>
            ...
            >
              ...
              <span>
                👁️ देखें कि ग्राहक को क्या दिखेगा (Preview Public View)
              </span>
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mt-6 pt-6 border-t border-slate-800">
          ...
          >
            ...
            <span>
              🛍️ मेरे प्रोडक्ट्स एवं रेट लिस्ट (My Products -{" "}
              {userProducts.length})
            </span>
          </button>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-xl space-y-6">
          <div>
            ...
            <h2 className="text-xl font-black text-slate-900 dark:text-white mt-2">
              कंट्रोल करें कि जब आप किसी को अपना डैशबोर्ड/लिंक भेजें तो उसे
              क्या-क्या दिखाई दे
            </h2>
            <p className="text-xs text-slate-500 mt-1">
              आप नीचे दिए गए विकल्पों में से जिसे ON रखेंगे, वही जानकारी आपके
              सार्वजनिक लिंक खोलने वाले ग्राहक या वेंडर को दिखाई देगी।
            </p>
          </div>
          ...
          <div className="p-5 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-2">
            ...
            <input
              ...
              onChange={(e) =>
                setShareSettings({
                  ...shareSettings,
                  headlineMessage: e.target.value,
                })
              }
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-xl space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
            <div>
              ...
              <p className="text-xs text-slate-500">
                यहाँ अपने नए सामान जोड़ें, दाम सेट करें एवं इन-स्टॉक स्टेटस टॉगल
                करें।
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {userProducts.map((p) => (
              <div
                key={p.id}
                className="bg-slate-50 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 space-y-3 flex flex-col justify-between"
              >
                <div className="space-y-2">
                  <div className="h-36 rounded-xl bg-slate-200 dark:bg-slate-700 overflow-hidden relative">
                    ...
                    />
                    <span
                      className={`absolute top-2 left-2 px-2 py-0.5 font-extrabold text-[10px] rounded-md text-white ${p.inStock ? "bg-emerald-600" : "bg-rose-600"}`}
                    >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 relative z-10">
              <div>
                ...
                <h2 className="text-xl font-black text-white mt-1 flex items-center gap-2">
                  ...
                  आपके लिए विशेष अधिकृत टूल एवं डैशबोर्ड सेवाएं (Authorized
                  Modules)
                </h2>
                <p className="text-xs text-slate-300">
                  सुपर एडमिन द्वारा आपके खाते ({activeUser.name}) के लिए
                  कस्टमाइज्ड किए गए डैशबोर्ड टूल्स एवं अनुमतियां
                </p>
              </div>

              <div className="bg-white/10 backdrop-blur-md px-3 py-2 rounded-2xl border border-white/20 text-right shrink-0">
                <span className="text-[10px] text-indigo-200 uppercase font-black block">
                  Role Access Level
                </span>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span
                        className={`px-2 py-0.5 text-[9px] font-black rounded-md flex items-center gap-1 ${
                          isAllowed
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
                  Enterprise Projects &amp; Cost Analytics
                </h1>
              </div>
              <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                Manage LiDAR surveys, civil BOQs, solar installations, interior
                budgets and spending trends in real-time.
              </p>
            </div>
          ...
          <div className="bg-white dark:bg-slate-800/90 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm space-y-6">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-bold text-slate-900 dark:text-white">
                    Historical Project Cost &amp; Spending Trends
                  </h2>
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                  Track monthly billing variances across Civil, Solar, Interior
                  &amp; LiDAR operations in ₹ Lakhs.
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-slate-50 dark:bg-slate-900/60 p-3.5 rounded-xl border border-slate-100 dark:border-slate-800">
                ...
                <div
                  className={`text-lg font-extrabold mt-1 flex items-center gap-1 ${
                ...
                <div className="space-y-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <button
                      onClick={() =>
                        alert(
                          `Opening full project report for ${proj.title}...`,
                        )
                      }
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4">
            <h3 className="font-black text-base text-slate-900 dark:text-white">
              नया प्रोडक्ट या सर्विस जोड़ें
            </h3>

            <form onSubmit={handleAddProduct} className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">
                  सामग्री का नाम (Title)
                </label>
                <input
                  type="text"
                  required
