...
export const LocationScopeBar: React.FC<LocationScopeBarProps> = ({
  ...
  const RADIUS_OPTIONS = [
    {
      value: 5,
      label: "5 Km",
      desc: "Local masons & nearby shops (Hyperlocal)",
    },
    { value: 10, label: "10 Km", desc: "City / Tehsil level vendors" },
    {
      value: 25,
      label: "25 Km",
      desc: "District level (Gorakhpur & surrounds)",
    },
    {
      value: 50,
      label: "50 Km",
      desc: "Regional belt (Campierganj, Basti, Deoria)",
    },
          ...
          <div className="absolute top-full left-0 mt-2 w-80 sm:w-96 bg-slate-900 border border-teal-500/40 rounded-3xl p-4 shadow-2xl z-50 text-white space-y-4 animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between border-b border-slate-800 pb-2">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-black text-xs text-white">
                  Location Scope Navigation (#location-scope)
                </h4>
              </div>
              <button
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-300 mb-1">
                🏙️ Select Metro / District Hub City:
              </label>
              <select
                value={selectedCity}
              ...
              >
                <option value="Gorakhpur">
                  Gorakhpur (Campierganj, GDA, AIIMS)
                </option>
                {INDIAN_CITIES.map((c) => (
            ...
            <div className="p-2.5 bg-teal-950/60 border border-teal-800/60 rounded-xl text-[11px] text-teal-200 flex items-center justify-between">
              <span>
                Active Scope:{" "}
                <strong>
                  {scopeMode.toUpperCase()} ({radiusKm} Km)
                </strong>
              </span>
  ...
  return (
    <div
      className={`bg-gradient-to-r from-slate-900 via-slate-850 to-slate-900 border border-teal-800/60 rounded-3xl p-4 sm:p-5 shadow-xl text-white space-y-4 ${className}`}
    >
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-300">
              Filter local masons, cement/steel dukandars, plumbers, &amp;
              ETP/STP dealers by distance radius
            </p>
          </div>
        </div>

        <div className="flex items-center gap-2">
          <span className="px-3 py-1 bg-teal-500/10 text-teal-300 rounded-full border border-teal-500/20 font-mono text-xs font-extrabold">
            {pinLocationLabel}
          </span>
        </div>
      </div>
