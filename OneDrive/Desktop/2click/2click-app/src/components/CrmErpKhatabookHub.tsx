...
export const CrmErpKhatabookHub: React.FC<CrmErpKhatabookHubProps> = ({
  ...
  const [activeSubModule, setActiveSubModule] = useState<
    "khatabook" | "crm" | "billing" | "attendance"
  >("khatabook");

  // KhataBook State
  const [khataAccounts, setKhataAccounts] = useState<KhataAccount[]>(() => {
    return getSafeLocalStorage<KhataAccount[]>(
      "2click_khata_accounts",
      INITIAL_KHATA_ACCOUNTS,
    );
  });
  const [selectedAccountId, setSelectedAccountId] = useState<string>(
    INITIAL_KHATA_ACCOUNTS[0].id,
  );
  ...
  const [txnPaymentMode, setTxnPaymentMode] = useState<
  ...
  const [newAccType, setNewAccType] = useState<
    "Customer" | "Dukandar" | "Supplier" | "Contractor"
  >("Customer");
  ...
  const [leadsList, setLeadsList] = useState<CrmLead[]>(() => {
    return getSafeLocalStorage<CrmLead[]>(
      "2click_crm_leads",
      INITIAL_CRM_LEADS,
    );
  });
  ...
  // Invoices State
  const [invoicesList, setInvoicesList] =
    useState<ErpInvoice[]>(INITIAL_INVOICES);
  ...
  const activeKhataAccount =
    khataAccounts.find((a) => a.id === selectedAccountId) || khataAccounts[0];
  ...
  const handleAddTransaction = (e: React.FormEvent) => {
    ...
    setTxnCategory("");
    showToast(
      `₹${amountNum.toLocaleString("en-IN")} ${txnType === "Gave" ? "Udhaar Added" : "Jama Payment Received"} successfully!`,
    );
  };
  ...
  const handleGpsCheckIn = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        setIsCapturingGps(false);
        showToast(
          `📍 Live GPS Attendance Tagged successfully at Lat: ${latitude.toFixed(4)}, Lng: ${longitude.toFixed(4)}`,
        );
      },
      (err) => {
        setIsCapturingGps(false);
        alert(
          "Could not fetch GPS coordinates. Please grant location permissions.",
        );
      },
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div>
            ...
            <p className="text-slate-300 text-xs sm:text-sm max-w-2xl mt-2 leading-relaxed">
              डिजिटल खाता बुक (उधार/जमा लेजर), ऑटोमैटिक व्हाट्सएप रिमाइंडर, GPS
              लोकेशन आधारित CRM लीड पाइपलाइन, GST इनवॉइस एवं फील्ड स्टाफ
              अटेंडेंस ट्रैकिंग का सम्पूर्ण समाधान।
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            ...
            >
              <Crosshair
                className={`w-4 h-4 ${isCapturingGps ? "animate-spin" : ""}`}
              />
              <span>
                {isCapturingGps
                  ? "GPS tagging..."
                  : "📍 GPS Attendance / Location Tag"}
              </span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-slate-800">
          <div className="bg-white/5 backdrop-blur-md p-3.5 rounded-2xl border border-white/10">
            <span className="text-[11px] text-emerald-300 font-bold block flex items-center gap-1">
              <TrendingUp className="w-3.5 h-3.5 text-emerald-400" /> आपको लेने
              हैं (Receivable)
            </span>
          ...
          <div className="bg-white/5 backdrop-blur-md p-3.5 rounded-2xl border border-white/10">
            <span className="text-[11px] text-rose-300 font-bold block flex items-center gap-1">
              <TrendingDown className="w-3.5 h-3.5 text-rose-400" /> आपको देने
              हैं (Payable)
            </span>
          ...
          <div className="bg-white/5 backdrop-blur-md p-3.5 rounded-2xl border border-white/10">
            <span className="text-[11px] text-indigo-300 font-bold block flex items-center gap-1">
              <Briefcase className="w-3.5 h-3.5 text-indigo-400" /> CRM Active
              Leads
            </span>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-3">
        ...
        <span className="text-xs font-bold text-slate-500">
          City Context:{" "}
          <strong className="text-teal-600">{selectedCity}</strong>
        </span>
      </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-200 dark:border-slate-800 shadow-xl space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-1 overflow-x-auto pb-1 scrollbar-none">
                {["All", "Customer", "Dukandar", "Supplier", "Contractor"].map(
                  (type) => (
                    <button
                      key={type}
              ...
              {khataAccounts
                .filter((acc) => {
                  const matchSearch =
                    acc.name
                      .toLowerCase()
                      .includes(khataSearchQuery.toLowerCase()) ||
                    acc.phone.includes(khataSearchQuery);
                  const matchFilter =
                    khataFilterType === "All" ||
                    acc.accountType === khataFilterType;
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-2">
                  ...
                  {activeKhataAccount.phone}
                  {activeKhataAccount.address && (
                    <span>• {activeKhataAccount.address}</span>
                  )}
                </p>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-right">
                ...
                >
                  ₹
                  {Math.abs(activeKhataAccount.netBalanceINR).toLocaleString(
                    "en-IN",
                  )}
                </span>
              ...
              {activeKhataAccount.transactions.length === 0 ? (
                <div className="p-8 text-center bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
                  ...
                  <p className="text-[11px] text-slate-400 mt-0.5">
                    Click "You Gave ₹" or "You Got ₹" to log first transaction.
                  </p>
                </div>
              ) : (
                <div className="divide-y divide-slate-100 dark:divide-slate-800 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden">
                  {activeKhataAccount.transactions.map((txn) => (
                    <div
                      key={txn.id}
                      className="p-4 bg-slate-50/50 dark:bg-slate-800/30 hover:bg-slate-100/60 transition flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs"
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          <span
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl">
            <div>
              ...
              <p className="text-xs text-slate-500">
                सोलर, सिविल बीओक्यू एवं आर्किटेक्चरल इंक्वायरी का स्टेज-वाइज लीड
                प्रबंधन एवं साइट जीपीएस इंस्पेक्शन रिकॉर्ड।
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              return (
                <div
                  key={stageName}
                  className="bg-slate-100 dark:bg-slate-900 p-4 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-3"
                >
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-xl space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
            <div>
              ...
              <p className="text-xs text-slate-500">
                सिविल वर्क्स, सोलर किट एवं इलेक्ट्रिकल सप्लाई के लिए अधिकृत
                जीएसटी टैक्स इनवॉइस बनाएं।
              </p>
            </div>

            <button
              onClick={() =>
                showToast("Invoicing system loaded in draft mode.")
              }
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden">
            {invoicesList.map((inv) => (
              <div
                key={inv.id}
                className="p-4 bg-slate-50/50 dark:bg-slate-800/30 hover:bg-slate-100/60 transition flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs"
              >
                <div>
                  <div className="flex items-center gap-2">
                ...
                <div className="text-right">
                  ...
                  <span className="text-[10px] text-slate-400 block">
                    Subtotal: ₹{inv.subtotalINR.toLocaleString("en-IN")} + GST:
                    ₹{inv.gstAmountINR.toLocaleString("en-IN")}
                  </span>
                </div>
              </div>
            ))}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-xl space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
            <div>
              ...
              <p className="text-xs text-slate-500">
                फील्ड कर्मचारियों एवं ठेकेदारों की ऑन-साइट लाइव जीपीएस अटेंडेंस
                तथा विजिट रिकॉर्ड।
              </p>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {attendanceRecords.map((rec) => (
                <div
                  key={rec.id}
                  className="p-4 bg-slate-50 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs"
                >
                  <div>
                    <span className="font-extrabold text-slate-900 dark:text-white block">
                      {rec.userName} ({rec.userRole})
                    </span>
                    <span className="text-[11px] text-slate-500 flex items-center gap-1 mt-0.5">
                      <Clock className="w-3 h-3 text-teal-600" />{" "}
                      {rec.timestamp} • {rec.locationName}
                    </span>
                  </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 max-w-md w-full shadow-2xl space-y-4 animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <h3 className="font-black text-base text-slate-900 dark:text-white flex items-center gap-2">
                ...
                {txnType === "Gave"
                  ? "🔴 You Gave Udhaar (उधार दिया)"
                  : "🟢 You Got Payment (जमा लिया)"}
              </h3>
              <button
            ...
            <form onSubmit={handleCreateLead} className="space-y-3 text-xs">
              ...
              <div>
                ...
                >
                  ...
                  <option value="Civil BOQ & Construction">
                    Civil BOQ &amp; Construction
                  </option>
                  <option value="Interior Modular & VR">
                    Interior Modular &amp; VR
                  </option>
                  <option value="Water ETP/STP Plant">
                    Water ETP/STP Plant
                  </option>
