...
export const DISTRICT_MAP_NODES: DistrictGeoData[] = [
  ...
  },
  {
    ...
    keyVendors: [
      {
        name: "Avadh Hardware & Solar",
        role: "Dukandar",
        phone: "+91 94150 11223",
        rating: 4.7,
      },
      {
        name: "Awadh MEP Services",
        role: "Electrician",
  ...
  },
  {
    ...
    keyVendors: [
      {
        name: "Kashi Smart Solar Hub",
        role: "Dukandar",
        phone: "+91 98390 44556",
        rating: 4.8,
      },
      {
        name: "Purvanchal MEP Works",
        role: "Plumber",
  ...
  },
  {
    ...
    keyVendors: [
      {
        name: "Charminar Electrical B2B",
        role: "Dukandar",
        phone: "+91 98490 12345",
        rating: 4.8,
      },
      {
        name: "Deccan Solar Grid",
        role: "Vendor",
...
export const DistrictInteractiveMap: React.FC<DistrictInteractiveMapProps> = ({
  ...
  const [activeDistrictNode, setActiveDistrictNode] = useState<DistrictGeoData>(
    DISTRICT_MAP_NODES[0],
  );
  ...
  useEffect(() => {
    ...
    const filteredNodes = DISTRICT_MAP_NODES.filter((node) => {
      const matchState =
        selectedStateFilter === "All" || node.state === selectedStateFilter;
      const matchSearch =
        searchQuery === "" ||
    ...
    filteredNodes.forEach((node) => {
      ...
      if (showHeatCircles) {
        ...
        const heatCircle = L.circle([node.lat, node.lng], {
          radius: radiusMeters,
          color:
            node.district === activeDistrictNode.district
              ? "#6366f1"
              : "#0d9488",
          fillColor:
            node.district === activeDistrictNode.district
  ...
  const handleDetectUserLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        setIsLocating(false);
        setLocationError(
          "Unable to retrieve your location. Check browser location permissions.",
        );
  ...
  const totalProjectsSum = DISTRICT_MAP_NODES.reduce(
    (acc, curr) => acc + curr.projectsCount,
    0,
  );
  const totalVendorsSum = DISTRICT_MAP_NODES.reduce(
    (acc, curr) => acc + curr.vendorsCount,
    0,
  );
  const totalUsersSum = DISTRICT_MAP_NODES.reduce(
    (acc, curr) => acc + curr.usersCount,
    0,
  );
  const totalValuationSum = DISTRICT_MAP_NODES.reduce(
    (acc, curr) => acc + curr.totalValuationINR,
    0,
  );
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 relative z-10">
          <div>
            ...
            <p className="text-slate-300 text-xs max-w-2xl mt-1">
              Real-time geospatial distribution of active construction, solar
              rooftop, MEP vendors, and registered users across Indian districts
              with live GPS positioning.
            </p>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
          ...
          >
            ...
            <span>
              District Density Bubbles ({showHeatCircles ? "ON" : "OFF"})
            </span>
          </button>
        </div>
      </div>
            ...
            <div className="p-3 bg-indigo-50/70 dark:bg-indigo-950/30 rounded-2xl border border-indigo-200 dark:border-indigo-800/80 space-y-1.5">
              ...
              <div className="flex items-center justify-between text-xs">
                <span className="font-extrabold text-slate-900 dark:text-white">
                  {activeDistrictNode.adminName}
                </span>
                <a
                  href={`tel:${activeDistrictNode.adminPhone}`}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs font-bold text-slate-700 dark:text-slate-300">
                ...
                <span className="text-emerald-600 dark:text-emerald-400 font-black">
                  ₹
                  {(activeDistrictNode.totalValuationINR / 10000000).toFixed(2)}{" "}
                  Cr
                </span>
              </div>
            ...
            <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
              ...
              <div className="space-y-1.5">
                {activeDistrictNode.keyVendors.map((vendor, idx) => (
                  <div
                    key={idx}
                    className="p-2.5 bg-slate-50 dark:bg-slate-800/80 rounded-xl border border-slate-200/80 dark:border-slate-700/80 text-xs flex items-center justify-between"
                  >
                    <div>
                      ...
                      <span className="text-[10px] text-teal-600 dark:text-teal-400 font-bold">
                        {vendor.role} • ★ {vendor.rating}
                      </span>
                    </div>
                    <a
                      href={`tel:${vendor.phone}`}
                      className="p-1.5 bg-teal-100 dark:bg-teal-950 text-teal-800 dark:text-teal-300 rounded-lg font-bold text-[10px] flex items-center gap-1"
                    >
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      <span
                        className={`text-[10px] ${isCur ? "text-indigo-200" : "text-slate-400"}`}
                      >
