...
export const ProjectGanttTimelineView: React.FC<
  ProjectGanttTimelineViewProps
> = ({ currentUser, selectedCity = "Gorakhpur" }) => {
  const [selectedProjectId, setSelectedProjectId] = useState<string>(
    SAMPLE_GANTT_PROJECTS[0].id,
  );
  const [categoryFilter, setCategoryFilter] = useState<string>("All");
  const [statusFilter, setStatusFilter] = useState<string>("All");

  const activeProject =
    SAMPLE_GANTT_PROJECTS.find((p) => p.id === selectedProjectId) ||
    SAMPLE_GANTT_PROJECTS[0];
  ...
  const filteredMilestones = activeProject.milestones.filter((m) => {
  ...
  const ganttChartData = filteredMilestones.map((m) => {
    ...
    return {
      id: m.id,
      taskName:
        m.taskName.length > 28 ? m.taskName.slice(0, 26) + "…" : m.taskName,
  ...
  const completedCount = activeProject.milestones.filter(
    (m) => m.status === "Completed",
  ).length;
  const inProgressCount = activeProject.milestones.filter(
    (m) => m.status === "In Progress",
  ).length;
  const delayedCount = activeProject.milestones.filter(
    (m) => m.status === "Delayed",
  ).length;
    ...
    <div className="space-y-8 animate-fade-in">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="px-3 py-1 bg-teal-500/20 text-teal-300 border border-teal-400/30 rounded-xl text-xs font-black uppercase tracking-wider flex items-center gap-1.5">
                <Calendar className="w-3.5 h-3.5" /> Construction Milestone
                Engine
              </span>
              <span
                className={`px-2.5 py-1 text-xs font-extrabold rounded-xl border ${
                  activeProject.status === "On Schedule"
                    ? "bg-emerald-500/20 text-emerald-300 border-emerald-400/30"
                    : activeProject.status === "Ahead of Schedule"
            ...
            <p className="text-xs sm:text-sm text-slate-300 max-w-2xl leading-relaxed">
              Track project duration, milestone dates, dependencies, and
              critical path delays with real-time Recharts visual progress
              tracking.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
            <div className="bg-slate-800/80 p-1.5 rounded-2xl border border-slate-700">
              <label className="block text-[10px] text-slate-400 font-bold px-2 mb-1">
            ...
            <button
              onClick={() => {
                ...
                alert(
                  `Exporting Gantt Timeline Schedule for ${activeProject.projectName} (PDF / MS Project)...`,
                );
              }}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between space-y-2">
          ...
          <div>
            <div className="text-xl font-black text-slate-900 dark:text-white font-mono">
              Day {activeProject.currentDayOffset}{" "}
              <span className="text-xs font-normal text-slate-400">
                / {activeProject.totalDurationDays} Days
              </span>
            </div>
        ...
        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between space-y-2">
          ...
          <div>
            <div
              className={`text-xl font-black ${delayedCount > 0 ? "text-rose-600 dark:text-rose-400" : "text-emerald-600 dark:text-emerald-400"}`}
            >
              {delayedCount > 0
                ? `${delayedCount} Delayed Milestone`
                : "0 Bottlenecks Detected"}
            </div>
            <div className="text-xs font-semibold text-slate-500 mt-1">
              {delayedCount > 0
                ? "Requires immediate supervisor intervention"
                : "All critical path items moving on schedule"}
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Horizontal timeline showing milestone start, duration, and
              completion progress.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              <option value="Approvals & Inspection">
                Approvals & Inspection
              </option>
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <ReferenceLine
                ...
                strokeDasharray="4 4"
                label={{
                  value: `Today (Day ${activeProject.currentDayOffset})`,
                  fill: "#f59e0b",
                  fontSize: 10,
                  position: "top",
                }}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Assigned contractor details, target deadlines, completion % and
              dependencies
            </p>
          </div>
