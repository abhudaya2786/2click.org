...
export type UnitType =
  "kitchen" | "wardrobe" | "tv_unit" | "false_ceiling" | "bathroom";
...
export type SurfaceFinish =
  ...
  | "2mm Anti-Scratch Acrylic"
  | "Natural Teak Veneer with PU Polish"
  | "Lacquered Glass";
export type HardwareBrand =
  "Standard Soft-Close" | "Hettich Germany" | "Hafele" | "Ebco Systems";
export type CountertopType =
  | "Quartz Stone (18mm Nano Quartz)"
  | "Jet Black Granite Slab"
  | "Profile LED Channel"
  | "Wooden Louvered Panels"
  | "Metallic Fluted Panels";
...
const AVAILABLE_UPGRADES: UpgradeOption[] = [
  {
    ...
    priceINR: 4500,
    description:
      "Motion activated under-cabinet diffuse warm white 3000K LED channel strip",
    applicableUnits: [
      "kitchen",
      "wardrobe",
      "tv_unit",
  ...
  },
  {
    ...
    priceINR: 3200,
    description:
      "Stainless steel 304 adjustable cutlery divider with spice jar rack",
  ...
  },
  {
    ...
    priceINR: 14500,
    description:
      "Heavy duty chrome basket blind corner pull-out carousel for maximum storage",
    applicableUnits: ["kitchen"],
  },
  {
    id: "upg-pantry-tall",
    ...
    priceINR: 22000,
    description:
      "Full height soft-close pantry pull-out frame with 6 German chrome wire baskets",
    applicableUnits: ["kitchen"],
  },
  {
    id: "upg-blum-hinges",
    ...
    priceINR: 6800,
    description:
      "Austrian Blumotion 110-degree clip-top soft-close concealed hinges",
  ...
  },
  {
    ...
    priceINR: 5500,
    description:
      "Reinforced heat resistant frame with rear ventilation for built-in appliances",
  ...
  },
  {
    ...
    priceINR: 8900,
    description:
      "Single bowl handmade 1.2mm SS304 satin sink with 360-degree brass sprayer faucet",
  ...
  },
  {
    ...
    priceINR: 4200,
    description:
      "Anodized gold champagne brass profile handles and fluted edge banding",
  ...
  },
  {
    ...
    priceINR: 6500,
    description:
      "Dimmable dual-color LED backlight with heated anti-fog touch demister pad",
...
export const InteriorStudio: React.FC = () => {
  ...
  const [previewMode, setPreviewMode] = useState<"3d_visual" | "2d_map">(
    "3d_visual",
  );
  const [activePresetTheme, setActivePresetTheme] = useState<string>(
    "Royal Indian Luxury",
  );

  // Camera and Lighting controls for 3D Preview
  const [cameraAngle, setCameraAngle] = useState<
    "perspective" | "front" | "isometric"
  >("perspective");
  const [underCabinetLightsOn, setUnderCabinetLightsOn] =
  ...
  const [unitConfigs, setUnitConfigs] = useState<
    Record<UnitType, RoomUnitConfig>
  >({
    kitchen: {
      id: "kitchen",
  ...
  const [enquirySuccessToast, setEnquirySuccessToast] = useState<string | null>(
    null,
  );

  // Custom Line Items with Image Attachments
  const [isAddModalOpen, setIsAddModalOpen] = useState<boolean>(false);
  const [customInteriorItems, setCustomInteriorItems] = useState<
    Array<{
      id: string;
      title: string;
      category: string;
  ...
  const getCarcassCostAdj = (carcass: CarcassMaterial): number => {
    switch (carcass) {
      case "BWP Marine Plywood (IS:710)":
        return 220;
      case "HDMR Board":
        return 180;
      case "Particle Board":
        return -100;
      default:
        return 0; // Commercial Ply
    }
  };

  const getFinishCostAdj = (finish: SurfaceFinish): number => {
    switch (finish) {
      case "2mm Anti-Scratch Acrylic":
        return 280;
      case "Natural Teak Veneer with PU Polish":
        return 450;
      case "Lacquered Glass":
        return 520;
      default:
        return 0; // 1mm Laminate
    }
  };

  const getHardwareCostAdj = (hardware: HardwareBrand): number => {
    switch (hardware) {
      case "Hettich Germany":
        return 160;
      case "Hafele":
        return 190;
      case "Ebco Systems":
  ...
  const calculateUnitCost = (config: RoomUnitConfig) => {
    ...
    const effectiveRatePerSqft =
      baseRate + carcassAdj + finishAdj + hardwareAdj;
    const baseAreaCost = Math.round(
      effectiveRatePerSqft * config.defaultAreaSqft,
    );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Configure modular kitchens, lacquered wardrobes, false ceilings
            &amp; vanities with 2D floorplan layout maps and 3D visual preview
            (&quot;कैसा लगेगा&quot; engine).
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="bg-gradient-to-r from-violet-600 to-indigo-600 text-white px-5 py-2.5 rounded-2xl shadow-md flex items-center gap-3">
            <div>
              <div className="text-[10px] text-violet-200 uppercase font-black tracking-wider">
                Total Interior Estimate
              </div>
      ...
      <div className="bg-white dark:bg-slate-800/90 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
        ...
        <div className="flex flex-wrap gap-2">
          {[
            {
              name: "Royal Indian Luxury",
              desc: "Teak Veneer + Brass Trim + Quartz",
            },
            {
              name: "Modern Minimalist",
              desc: "High Gloss White Acrylic + LED",
            },
            {
              name: "Urban Industrial",
              desc: "Lacquered Charcoal Glass + Louvers",
            },
            {
              name: "Scandi Wood & Pastel",
              desc: "Warm Laminate + Fluted Panels",
            },
          ].map((theme) => (
            <button
              key={theme.name}
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-700">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Select material grades, shutter finishes, hardware brands
                  &amp; upgrades
                </p>
              </div>
              ...
              <div className="space-y-1.5 p-3 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700">
                ...
                <select
                  ...
                  onChange={(e) => {
                    setUnitConfigs({
                      ...
                      [activeUnit]: {
                        ...activeConfig,
                        carcass: e.target.value as CarcassMaterial,
                      },
                    });
                  }}
                ...
                >
                  <option value="Commercial Ply">
                    Commercial Ply (MR Grade Base)
                  </option>
                  <option value="BWP Marine Plywood (IS:710)">
                    BWP Marine Plywood IS:710 (+ ₹220/sqft)
                  </option>
                  <option value="HDMR Board">
                    HDMR Board (Action TESA) (+ ₹180/sqft)
                  </option>
                  <option value="Particle Board">
                    Particle Board (- ₹100/sqft)
                  </option>
                </select>
                <p className="text-[10px] text-slate-500">
                  Internal box frame and shelf load-bearing material.
                </p>
              </div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    setUnitConfigs({
                      ...
                      [activeUnit]: {
                        ...activeConfig,
                        finish: e.target.value as SurfaceFinish,
                      },
                    });
                  }}
                  className="w-full p-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl font-bold text-slate-900 dark:text-white"
                >
                  <option value="11mm High-Pressure Laminate">
                    1mm High-Pressure Laminate (Standard Matt/SF)
                  </option>
                  <option value="2mm Anti-Scratch Acrylic">
                    2mm Anti-Scratch Acrylic Gloss (+ ₹280/sqft)
                  </option>
                  <option value="Natural Teak Veneer with PU Polish">
                    Natural Teak Veneer with PU Polish (+ ₹450/sqft)
                  </option>
                  <option value="Lacquered Glass">
                    Lacquered Glass with Aluminium (+ ₹520/sqft)
                  </option>
                </select>
                <p className="text-[10px] text-slate-500">
                  External visible door shutter look and touch finish.
                </p>
              </div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    setUnitConfigs({
                      ...
                      [activeUnit]: {
                        ...activeConfig,
                        hardware: e.target.value as HardwareBrand,
                      },
                    });
                  }}
                  className="w-full p-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl font-bold text-slate-900 dark:text-white"
                >
                  <option value="Standard Soft-Close">
                    Standard Soft-Close Hinges
                  </option>
                  <option value="Hettich Germany">
                    Hettich Germany (Sensys &amp; Atira) (+ ₹160/sqft)
                  </option>
                  <option value="Hafele">
                    Hafele Germany (Free Flap &amp; Matrix) (+ ₹190/sqft)
                  </option>
                  <option value="Ebco Systems">
                    Ebco Tandem &amp; Carousel (+ ₹110/sqft)
                  </option>
                </select>
                <p className="text-[10px] text-slate-500">
                  Soft-close hinges, drawer runners &amp; lift-up mechanisms.
                </p>
              </div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    setUnitConfigs({
                      ...
                      [activeUnit]: {
                        ...activeConfig,
                        countertop: e.target.value as CountertopType,
                      },
                    });
                  }}
                  className="w-full p-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl font-bold text-slate-900 dark:text-white"
                >
                  <option value="Quartz Stone (18mm Nano Quartz)">
                    Quartz Stone (18mm Nano Quartz)
                  </option>
                  <option value="Jet Black Granite Slab">
                    Jet Black Granite Slab
                  </option>
                  <option value="Profile LED Channel">
                    Profile LED Channel Lighting
                  </option>
                  <option value="Wooden Louvered Panels">
                    Wooden Louvered Panels
                  </option>
                  <option value="Metallic Fluted Panels">
                    Metallic Fluted Panels
                  </option>
                </select>
                <p className="text-[10px] text-slate-500">
                  Top work surface or vertical wall feature material.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-3 border-t border-slate-100 dark:border-slate-700">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                {AVAILABLE_UPGRADES.filter((u) =>
                  u.applicableUnits.includes(activeUnit),
                ).map((upgrade) => {
                  const isChecked = activeConfig.selectedUpgrades.includes(
                    upgrade.id,
                  );
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-700">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h3 className="font-bold text-xs text-slate-900 dark:text-white flex items-center gap-2">
                    ...
                    6. Bespoke Interior Items &amp; Attachments (
                    {customInteriorItems.length})
                  </h3>
                  <p className="text-[11px] text-slate-500">
                    Attach photos of custom shutters, wallpapers, solid teak
                    louvers, or designer lighting
                  </p>
                </div>
                <button
                  type="button"
              ...
              {customInteriorItems.length > 0 ? (
                ...
                <div className="p-4 rounded-xl border border-dashed border-slate-300 dark:border-slate-700 text-center text-xs text-slate-500">
                  No custom line items added yet. Click &quot;Add Custom
                  Item&quot; to upload photo and set price.
                </div>
              )}
            </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-900 text-white flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-xs">
              ...
              <div className="flex items-center gap-3 text-[11px] text-slate-300">
                <div>
                  Base Area: ₹
                  {activeCostData.baseAreaCost.toLocaleString("en-IN")}
                </div>
                <div>
                  + Upgrades: ₹
                  {activeCostData.upgradesCost.toLocaleString("en-IN")}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-md space-y-4 sticky top-24">
              ...
              <div className="space-y-4">
                ...
                <div className="relative rounded-2xl overflow-hidden bg-slate-950 border border-slate-800 h-80 shadow-inner flex flex-col justify-between p-4">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center p-6 overflow-hidden">
                    ...
                    <div
                      ...
                      style={{
                        ...
                        boxShadow: underCabinetLightsOn
                  ...
                  <div className="z-10 flex justify-between items-end text-white text-[10px]">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() =>
                          setUnderCabinetLightsOn(!underCabinetLightsOn)
                        }
                      ...
                      >
                        ...
                        <span>
                          Profile LED {underCabinetLightsOn ? "ON" : "OFF"}
                        </span>
                      </button>
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 space-y-2 text-xs">
                  <div className="flex justify-between items-center font-bold text-slate-900 dark:text-white">
                    ...
                    <span className="text-[10px] px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded font-mono">
                      Verified B2B Grade
                    </span>
                  </div>
                  <ul className="space-y-1 text-slate-600 dark:text-slate-300 text-[11px] list-disc list-inside">
                    ...
                    <li>
                      ...
                      {activeConfig.hardware}
                    </li>
                    <li>
                      <strong className="text-slate-900 dark:text-white">
                        Countertop:
                      </strong>{" "}
                      {activeConfig.countertop}
                    </li>
                  </ul>
                </div>
              </div>
              ...
              <div className="space-y-4">
                  ...
                  <div className="my-auto border-2 border-dashed border-violet-500/50 rounded-2xl p-4 bg-slate-950/80 space-y-3">
                    <div className="flex justify-between text-[10px] text-slate-400 font-mono">
                      <span>
                        ◄ Wall Length:{" "}
                        {Math.round(
                          Math.sqrt(activeConfig.defaultAreaSqft) * 1.3,
                        )}{" "}
                        Feet ►
                      </span>
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs space-y-2">
                  ...
                  <p className="text-slate-600 dark:text-slate-300 text-[11px] leading-relaxed">
                    Designed according to ergonomic work triangle standards for
                    maximum efficiency. Includes pre-marked electrical sockets,
                    plumbing drainage conduits, and soft-close shutter swing
                    allowances.
                  </p>
                </div>
              </div>
            ...
            <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white space-y-3">
              ...
              <div className="space-y-1.5 text-xs text-slate-300">
                {Object.values(unitConfigs).map((c) => (
                  <div key={c.id} className="flex justify-between items-center">
                    <span>
                      {c.title} ({c.defaultAreaSqft} sqft):
                    </span>
                    <span className="font-bold text-white">
                      ₹
                      {calculateUnitCost(c).totalUnitCost.toLocaleString(
                        "en-IN",
                      )}
                    </span>
                  </div>
              ...
              <div className="pt-3 border-t border-slate-700 flex justify-between items-center">
                <div>
                  <span className="text-[10px] text-violet-300 uppercase font-black block">
                    Grand Total
                  </span>
        ...
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl max-w-lg w-full border border-violet-500 shadow-2xl space-y-5 text-xs">
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-700 pb-3">
              ...
              <button
                onClick={() => setEnquiryModalOpen(null as any)}
            ...
            <div className="p-4 rounded-2xl bg-violet-50 dark:bg-violet-950/40 border border-violet-200 dark:border-violet-800 space-y-2">
              ...
              <div className="text-base font-black text-violet-600 dark:text-violet-400">
                Total Estimate: ₹{grandTotalINR.toLocaleString("en-IN")} (Incl.
                GST)
              </div>
              <p className="text-[11px] text-slate-600 dark:text-slate-300">
                Includes detailed breakdown for Modular Kitchen, Bedroom
                Wardrobes, False Ceilings, TV Unit &amp; Bath Vanities.
              </p>
            </div>
            ...
            <form
              onSubmit={(e) => {
                ...
                setEnquirySuccessToast(
                  `Interior BOQ Inquiry sent successfully! Verified contractors will contact you with site measurement dates.`,
                );
      ...
      {enquirySuccessToast && (
        <div className="fixed bottom-6 right-6 z-50 max-w-md bg-emerald-900 text-white p-4 rounded-2xl border border-emerald-500 shadow-2xl flex items-center justify-between gap-3 text-xs">
          ...
          <button
            onClick={() => setEnquirySuccessToast(null)}
            className="p-1 hover:bg-emerald-800 rounded-lg"
          >
