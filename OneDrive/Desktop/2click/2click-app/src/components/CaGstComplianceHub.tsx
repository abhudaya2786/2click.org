...
const INITIAL_VAULT_DOCS: VaultDocument[] = [
  {
    ...
    notes:
      "Official GST registration certificate issued by Govt of India for Sri Ram Traders & Civil Contractors.",
  },
  {
    id: "vault_2",
    ...
    notes:
      "Acknowledged GSTR-3B return receipt with tax payment reference #2C-PAY-99182.",
  },
  {
    id: "vault_3",
    ...
    notes:
      "Classified under Small Enterprise category eligible for bank interest subsidies.",
  },
  {
    id: "vault_4",
  ...
  },
  {
    ...
    notes:
      "Downloaded directly from TRACES portal for Q1 contractor deductions.",
  },
  {
    id: "vault_6",
    ...
    notes:
      "Official audited financial statement signed by chartered accountant for bank loan limits.",
  },
  {
    id: "vault_7",
    ...
    notes:
      "Municipal trade license. Due for 3-year renewal before 31 March 2027.",
  },
];
...
const formatBoldText = (str: string) => {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith("*") && part.endsWith("*")) {
      return (
        <em key={i} className="italic text-amber-700 dark:text-amber-300">
          {part.slice(1, -1)}
        </em>
      );
    }
  ...
  return lines.map((line, idx) => {
    if (line.startsWith("### ")) {
      return (
        <h4
          key={idx}
          className="font-black text-slate-900 dark:text-slate-100 text-xs sm:text-sm mt-3 mb-1.5 flex items-center gap-1.5 border-b border-amber-500/20 pb-1"
        >
          {line.replace("### ", "")}
        </h4>
      );
    ...
    if (line.startsWith("- ") || line.startsWith("* ")) {
      return (
        <li
          key={idx}
          className="ml-4 list-disc text-xs text-slate-700 dark:text-slate-300 my-0.5"
        >
    ...
    if (/^\d+\.\s/.test(line)) {
      return (
        <div
          key={idx}
          className="text-xs text-slate-800 dark:text-slate-200 font-medium my-1 pl-2 border-l-2 border-amber-500/50"
        >
          {formatBoldText(line)}
        </div>
      );
    ...
    return (
      <p
        key={idx}
        className="text-xs text-slate-800 dark:text-slate-200 leading-relaxed my-0.5"
      >
        {formatBoldText(line)}
      </p>
    );
...
const CA_SERVICES = [
  ...
  },
  {
    ...
    popular: true,
    requiredDocs: [
      "Sales Invoices (Excel/PDF)",
      "Purchase Invoices (GSTR-2B)",
  ...
  },
  {
    ...
    requiredDocs: [
      "TAN Number",
      "TDS Deduction Challans (Form 281)",
      "Deductee PAN Details & Amount",
    ],
  },
  {
    id: "form16_gen",
    ...
    requiredDocs: [
      ...
      "Quarterly Filed Return Receipt",
    ],
  },
  {
    ...
    requiredDocs: [
      ...
      "Firm Name & Business Address Proof",
      "Electricity Bill",
    ],
  },
  {
  ...
  },
  {
    ...
    popular: true,
    requiredDocs: [
      "2 Directors PAN, Aadhaar & Passport Photo",
  ...
  },
  {
    ...
    requiredDocs: [
      "PAN Card",
      "Aadhaar Card",
      "Mobile & Email Video Verification",
    ],
  },
];
...
export const CaGstComplianceHub: React.FC<CaGstComplianceHubProps> = ({
  ...
  const [caChatMessages, setCaChatMessages] =
    useState<CaChatMessage[]>(INITIAL_CA_MESSAGES);
  ...
  const [vaultDocs, setVaultDocs] =
    useState<VaultDocument[]>(INITIAL_VAULT_DOCS);
  const [vaultCategory, setVaultCategory] = useState<
    "all" | "certificate" | "tax_receipt" | "financials" | "license" | "other"
  >("all");
  ...
  const [newDocCategory, setNewDocCategory] = useState<
    "certificate" | "tax_receipt" | "financials" | "license" | "other"
  >("certificate");
  ...
  const [calcSupplyType, setCalcSupplyType] = useState<"intra" | "inter">(
    "intra",
  );
  ...
  const [tdsSection, setTdsSection] = useState<
    "194C_ind" | "194C_comp" | "194J" | "194I_rent"
  >("194C_comp");
  ...
  const gstCalculations = useMemo(() => {
    ...
    return {
      ...
      netTaxLiability,
      excessItcRemaining:
        calcPurchaseItc > grossGstPayable
          ? calcPurchaseItc - grossGstPayable
          : 0,
    };
  ...
  const filteredVaultDocs = useMemo(() => {
    return vaultDocs.filter((doc) => {
      const matchesCategory =
        vaultCategory === "all" || doc.category === vaultCategory;
      const matchesSearch =
        ...
        (doc.docNumber &&
          doc.docNumber
            .toLowerCase()
            .includes(vaultSearchQuery.toLowerCase())) ||
        (doc.arnNumber &&
          doc.arnNumber
            .toLowerCase()
            .includes(vaultSearchQuery.toLowerCase())) ||
        doc.tags.some((t) =>
          t.toLowerCase().includes(vaultSearchQuery.toLowerCase()),
        );
  ...
  const handleDeleteVaultDoc = (docId: string) => {
    if (
      window.confirm(
        "क्या आप इस दस्तावेज को वॉल्ट से हटाना चाहते हैं? (Are you sure you want to delete this document?)",
      )
    ) {
  ...
  const handleAddVaultDoc = (e: React.FormEvent) => {
    ...
    const newDoc: VaultDocument = {
      ...
      docNumber:
        newDocNumber || `DOC-${Math.floor(100000 + Math.random() * 900000)}`,
      ...
      notes:
        newDocNotes ||
        "Uploaded directly by user to Compliance Documents Vault.",
    };

    setVaultDocs((prev) => [newDoc, ...prev]);
    setVaultSuccessMsg(
      `🎉 "${newDocTitle}" वॉल्ट में सफलतापूर्वक 256-Bit SSL एन्क्रिप्ट व स्टोर हो गया!`,
    );
  ...
  const handleSubmitOrder = (e: React.FormEvent) => {
    e.preventDefault();
    setOrderSuccessMsg(
      `🎉 सर्विस ऑर्डर सफलतापूर्वक दर्ज किया गया! ऑर्डर आईडी: #2C-CA-${Math.floor(100000 + Math.random() * 900000)}. हमारे असाइन्ड सीए (Chartered Accountant) आपसे व्हाट्सएप पर तुरंत संपर्क करेंगे।`,
    );
    setTimeout(() => {
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-slate-300 text-xs sm:text-sm max-w-3xl leading-relaxed">
              कहीं जाने की जरुरत नहीं! जीएसटी रजिस्ट्रेशन, मासिक रिटर्न (GSTR-1
              &amp; 3B), आईटीआर (ITR Filing), टीडीएस सर्टिफिकेट एवं नई
              फर्म/कंपनी रजिस्ट्रेशन - 100% डिजिटल, सुरक्षित व सीए द्वारा
              सत्यापित।
            </p>
          </div>
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-3 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-2 md:pb-0 scrollbar-none">
          {[
            ...
            {
              id: "vault",
              label: "डॉक्युमेंट वॉल्ट (Compliance Vault)",
              icon: FolderLock,
            },
            ...
            {
              id: "registration",
              label: "नई फर्म / कंपनी (Registration)",
              icon: Building2,
            },
            {
              id: "calculators",
              label: "टैक्स कैलकुलेटर (Calculators)",
              icon: Calculator,
            },
          ].map((tab) => {
            const Icon = tab.icon;
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  अपनी आवश्यकतानुसार सर्विस चुनें, दस्तावेज अपलोड करें और
                  एक्सपर्ट सीए द्वारा कार्य पूरा कराएं।
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
              ...
              <div>
                <h3 className="font-black text-slate-900 dark:text-slate-100 text-base">
                  1. जीएसटी टैक्स एवं आईटीसी कैलकुलेटर (GST &amp; ITC
                  Calculator)
                </h3>
                <p className="text-xs text-slate-500">
                  सेल टर्नओवर और इनपुट टैक्स क्रेडिट (ITC) एडजस्टमेंट दर जांचें
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    value={calcTurnover}
                    onChange={(e) =>
                      setCalcTurnover(parseFloat(e.target.value) || 0)
                    }
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  >
                    <option value="intra">
                      राज्य के अंदर (Intra-State: CGST+SGST)
                    </option>
                    <option value="inter">
                      राज्य के बाहर (Inter-State: IGST)
                    </option>
                  </select>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCalcPurchaseItc(parseFloat(e.target.value) || 0)
                    }
                    className="w-full pl-8 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl font-bold text-slate-900 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                  />
                </div>
              ...
              <div className="bg-slate-900 text-white p-5 rounded-2xl space-y-3 border border-slate-800">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400">
                    कुल देय जीएसटी (Gross Tax):
                  </span>
                  <span className="font-extrabold text-white">
                    ₹{gstCalculations.grossGstPayable.toLocaleString("en-IN")}
                  </span>
                </div>
                ...
                {calcSupplyType === "intra" ? (
                  <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-800 p-2 rounded-xl text-slate-300">
                    <div>
                      ...
                      <span className="font-bold text-white">
                        ₹{gstCalculations.cgst.toLocaleString("en-IN")}
                      </span>
                    </div>
                    <div>
                      SGST (9%):{" "}
                      <span className="font-bold text-white">
                        ₹{gstCalculations.sgst.toLocaleString("en-IN")}
                      </span>
                    </div>
                  </div>
                ) : (
                  <div className="text-[11px] bg-slate-800 p-2 rounded-xl text-slate-300">
                    IGST (18%):{" "}
                    <span className="font-bold text-white">
                      ₹{gstCalculations.igst.toLocaleString("en-IN")}
                    </span>
                  </div>
                )}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-md space-y-6">
            <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  ठेकेदार, प्रोफेशनल व रेंट भुगतान पर टीडीएस दर जांचें
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    value={tdsAmount}
                    onChange={(e) =>
                      setTdsAmount(parseFloat(e.target.value) || 0)
                    }
              ...
              <div>
                ...
                >
                  <option value="194C_ind">
                    194C - Contractor payment to Individual/HUF (1%)
                  </option>
                  <option value="194C_comp">
                    194C - Contractor payment to Pvt Ltd / Firm (2%)
                  </option>
                  <option value="194J">
                    194J - Professional / Engineer / Legal Fees (10%)
                  </option>
                  <option value="194I_rent">
                    194I - Office / Warehouse Building Rent (10%)
                  </option>
                </select>
              </div>
              ...
              <div className="bg-slate-900 text-white p-5 rounded-2xl space-y-3 border border-slate-800 mt-6">
                ...
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400">
                    लागू टीडीएस दर (TDS Rate):
                  </span>
                  <span className="font-extrabold text-white">
                    {tdsCalculations.rate}%
                  </span>
                </div>

                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400">
                    कटौती योग्य टीडीएस (TDS to Deduct):
                  </span>
                  <span className="font-extrabold text-amber-400">
                    ₹{tdsCalculations.calculatedTds.toLocaleString("en-IN")}
                  </span>
                </div>

                <div className="pt-2 border-t border-slate-800 flex justify-between items-center text-sm">
                  <span className="font-extrabold text-slate-200">
                    वेंडर को देने योग्य नेट राशि (Net to Pay):
                  </span>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 shadow-md space-y-8">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
            <div>
              ...
              <p className="text-xs text-slate-500">
                आपके द्वारा दर्ज की गई रिटर्न व कंपनी फाइलिंग की रियल-टाइम
                स्थिति
              </p>
            </div>
          ...
          <div className="p-5 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700/80 space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
              <div>
                ...
                <span className="text-slate-500">
                  दर्ज तिथि: 28 जुलाई | क्लाइंट: श्री राम ट्रेडर्स
                </span>
              </div>
            ...
            <div className="grid grid-cols-4 gap-2 pt-2">
              {[
                {
                  step: "1",
                  title: "दस्तावेज प्राप्त",
                  active: true,
                  done: true,
                },
                {
                  step: "2",
                  title: "सीए वेरिफिकेशन",
                  active: true,
                  done: true,
                },
                {
                  step: "3",
                  title: "पोर्टल पर फाइलिंग",
                  active: true,
                  done: false,
                },
                {
                ...
                <div key={idx} className="space-y-1 text-center">
                  <div
                    className={`h-2 rounded-full ${s.done ? "bg-emerald-500" : s.active ? "bg-amber-500 animate-pulse" : "bg-slate-200 dark:bg-slate-700"}`}
                  ></div>
            ...
            <div className="p-4 bg-indigo-950 text-white rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs font-extrabold text-white">
                    CA अनुज अग्रवाल (FCA - ICAI Reg: 512034)
                  </div>
                  <div className="text-[11px] text-indigo-300">
                    वरिष्ठ जीएसटी एवं टैक्स सलाहकार
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <button
                  onClick={() =>
                    window.open(
                      "https://wa.me/919845011223?text=Hello%20CA%20Sir%2C%20regarding%20my%20GST%20filing",
                      "_blank",
                    )
                  }
                ...
                <button
                  onClick={() =>
                    alert("📞 सीए हेल्पलाइन: +91 98450 11223 पर कॉल करें")
                  }
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-xl w-full p-6 sm:p-8 border border-slate-200 dark:border-slate-800 shadow-2xl relative space-y-6 my-8">
            ...
            <div className="space-y-1">
              ...
              <div className="text-xs font-bold text-emerald-600">
                शुल्क: ₹{selectedService.price.toLocaleString("en-IN")}{" "}
                (प्रोसेसिंग समय: {selectedService.timeframe})
              </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
              <div className="space-y-2">
                ...
                <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight flex items-center gap-3">
                  ...
                  <span>
                    अनुपालन डॉक्युमेंट वॉल्ट (Compliance Documents Vault)
                  </span>
                </h2>
                <p className="text-slate-300 text-xs sm:text-sm max-w-2xl leading-relaxed">
                  आपकी कंपनी का डिजिटल लॉकर - जीएसटी रजिस्ट्रेशन प्रमाणपत्र,
                  मासिक GSTR व ITR फाइलिंग रसीदें (ARN), टीडीएस सर्टिफिकेट (Form
                  16A) और सीए ऑडिटेड फाइनेंशियल रिपोर्ट सुरक्षित रखें।
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-slate-800 text-xs">
              <div className="p-3 bg-slate-800/80 rounded-2xl border border-slate-700">
                <div className="text-slate-400 text-[10px] uppercase font-bold">
                  कुल संग्रहीत डॉक्युमेंट्स
                </div>
                <div className="text-xl font-black text-white mt-0.5">
                  {vaultDocs.length} फाइल्स
                </div>
              </div>

              <div className="p-3 bg-slate-800/80 rounded-2xl border border-slate-700">
                <div className="text-slate-400 text-[10px] uppercase font-bold">
                  सीए सत्यापित (CA Verified)
                </div>
                <div className="text-xl font-black text-emerald-400 mt-0.5">
                  {vaultDocs.filter((d) => d.status === "verified").length} /{" "}
                  {vaultDocs.length}
                </div>
              </div>
              ...
              <div className="p-3 bg-slate-800/80 rounded-2xl border border-slate-700">
                ...
                <div className="text-xl font-black text-amber-300 mt-0.5">
                  {vaultDocs.filter((d) => d.status === "expiring_soon").length}{" "}
                  एक्टिव
                </div>
              </div>
            </div>
          ...
          {vaultLocked ? (
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 shadow-md text-center max-w-md mx-auto space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500">
                  कंपनी प्रमाणपत्र व फाइलिंग रसीदें देखने के लिए 4-अंकीय PIN
                  दर्ज करें। (डेमो पिन: 1234)
                </p>
              </div>
              ...
              <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs">
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto pb-2 lg:pb-0 scrollbar-none">
                  {[
                    {
                      id: "all",
                      label: `सभी (${vaultDocs.length})`,
                      icon: Layers,
                    },
                    {
                      id: "certificate",
                      label: `रजिस्ट्रेशन प्रमाणपत्र (${vaultDocs.filter((d) => d.category === "certificate").length})`,
                      icon: Building2,
                    },
                    {
                      id: "tax_receipt",
                      label: `रिटर्न व टीडीएस रसीदें (${vaultDocs.filter((d) => d.category === "tax_receipt").length})`,
                      icon: FileCheck,
                    },
                    {
                      id: "financials",
                      label: `ऑडिट व रिपोर्ट (${vaultDocs.filter((d) => d.category === "financials").length})`,
                      icon: FileText,
                    },
                    {
                      id: "license",
                      label: `लाइसेंस व परमिट (${vaultDocs.filter((d) => d.category === "license").length})`,
                      icon: Award,
                    },
                  ].map((tab) => {
                    const Icon = tab.icon;
                ...
                {filteredVaultDocs.length === 0 ? (
                  <div className="col-span-full py-12 text-center bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-3">
                    ...
                    <p className="text-xs text-slate-400">
                      अपनी खोज शब्द बदलें या "+ नया डॉक्युमेंट जोड़ें" बटन
                      दबाएं।
                    </p>
                  </div>
                ) : (
                    ...
                    >
                          ...
                          <span
                            className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider ${
                              doc.category === "certificate"
                                ...
                                : doc.category === "tax_receipt"
                                  ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20"
                                  : doc.category === "financials"
                      ...
                      <div className="flex flex-wrap gap-1">
                        {doc.tags.map((tag, idx) => (
                          <span
                            key={idx}
                            className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded text-[10px] font-bold"
                          >
                            #{tag}
                          </span>
                        ))}
                      ...
                      <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() =>
                              alert(
                                `📥 "${doc.title}" की PDF फाइल डाउनलोड शुरू हो गई है!`,
                              )
                            }
          ...
          <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white rounded-3xl p-6 sm:p-8 border border-slate-800 shadow-xl relative overflow-hidden">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
              <div className="space-y-2">
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-500/20 text-amber-300 border border-amber-500/30 rounded-full text-xs font-black uppercase tracking-wider">
                  <Sparkles className="w-4 h-4 text-amber-400" />
                  <span>
                    AI Chartered Accountant Assistant • Instant Advisory
                  </span>
                </div>
                <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight flex items-center gap-3">
                ...
                <p className="text-slate-300 text-xs sm:text-sm max-w-2xl leading-relaxed">
                  जीएसटी रजिस्ट्रेशन, लेट फीस पेनल्टी, इनपुट टैक्स क्रेडिट (ITC)
                  क्लेम, टीडीएस दरें या कंपोजिशन स्कीम से जुड़े किसी भी सवाल का
                  तुरंत कानूनी व सीए समाधान पाएं।
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 bg-slate-800/80 p-3.5 rounded-2xl border border-slate-700/80">
                ...
                <div>
                  <div className="text-xs font-black text-emerald-400">
                    2Click CA Desk AI Live
                  </div>
                  <div className="text-[10px] text-slate-400">
                    Senior FCA Model Connected
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 pt-6 border-t border-slate-800 space-y-2">
              <div className="text-[11px] font-black uppercase tracking-wider text-amber-400 flex items-center gap-1.5">
                ...
                <span>
                  अक्सर पूछे जाने वाले सवाल (Quick Compliance Questions):
                </span>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden flex flex-col h-[550px]">
            ...
            <div className="bg-slate-900 text-white p-4 border-b border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-slate-400">
                    CGST Act 2017, Income Tax Act 1961 &amp; Notification
                    Updates
                  </div>
                </div>
              </div>
              ...
              {isCaThinking && (
                <div className="flex items-center gap-3 p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 max-w-sm">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="text-[10px] text-slate-400">
                      Checking CGST Sections &amp; Latest GST Portal
                      Notifications
                    </div>
                  </div>
                </div>
            ...
            <div className="p-3 sm:p-4 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
              ...
              <div className="mt-2 text-[10px] text-slate-400 text-center">
                ⚠️ डिस्क्लेमर: यह एआई टूल केवल प्रारंभिक मार्गदर्शन प्रदान करता
                है। आधिकारिक रिटर्न फाइलिंग व सीए हस्ताक्षर के लिए सीए सर्विसेज
                अनुभाग से डायरेक्ट कंसल्टेंट बुक करें।
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-extrabold text-sm text-white">
                    सत्यापित अनुपालन दस्तावेज (Verified Document)
                  </h3>
                  <p className="text-[10px] text-slate-400">
                    2Click CA Vault Security Signed
                  </p>
                </div>
              </div>
            ...
            <div className="p-6 space-y-6">
              <div className="bg-gradient-to-b from-amber-50/40 via-white to-slate-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 border-4 border-double border-amber-500/40 rounded-2xl p-6 relative space-y-6 shadow-inner">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-slate-800">
                  <div className="space-y-1">
                    <div className="text-[10px] font-bold text-slate-400 uppercase">
                      सत्यापितकर्ता सीए (Verifying Authority):
                    </div>
                    ...
                    <div className="text-[10px] text-slate-400">
                      2Click Empanelled Chartered Accountant Unit
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-between gap-3 pt-2">
                <button
                  onClick={() =>
                    alert(`🖨️ "${previewDoc.title}" का प्रिंट कमांड दिया गया!`)
                  }
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() =>
                      alert(`📥 "${previewDoc.title}" डाउनलोड हो रहा है...`)
                    }
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 my-8">
            <div className="bg-slate-900 text-white p-5 flex items-center justify-between border-b border-slate-800">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-extrabold text-sm text-white">
                    वॉल्ट में नया डॉक्युमेंट जोड़ें
                  </h3>
                  <p className="text-[10px] text-slate-400">
                    256-Bit Encrypted Cloud Vault Storage
                  </p>
                </div>
              </div>
              ...
              {vaultSuccessMsg ? (
                ...
                <form onSubmit={handleAddVaultDoc} className="space-y-4">
                    ...
                    <div>
                      ...
                      >
                        <option value="certificate">
                          कंपनी / जीएसटी रजिस्ट्रेशन
                        </option>
                        <option value="tax_receipt">
                          रिटर्न फाइलिंग रसीद / ARN
                        </option>
                        <option value="financials">
                          ऑडिट बैलेंस शीट व P&amp;L
                        </option>
                  ...
                  >
                    ...
                    <span>
                      वॉल्ट में सुरक्षित सेव करें (Encrypt &amp; Save)
                    </span>
                  </button>
                </form>
              )}
