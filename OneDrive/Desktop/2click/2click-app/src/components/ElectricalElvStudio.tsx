...
export interface ElectricalJobItem {
  ...
  location: string;
  workType:
    "wiring" | "fitting" | "earthing" | "lighting" | "maintenance" | "testing";
  workTitle: string;
  pointsCount: number;
...
export const ElectricalElvStudio: React.FC<ElectricalElvStudioProps> = ({
  ...
  const [activeStudioMode, setActiveStudioMode] = useState<
    "electrician_dashboard" | "transformer_calculator"
  >("electrician_dashboard");
  ...
  const estimatedWiringLaborINR =
    calcPointsCount * ratePerPoint +
  ...
  const [jobsList, setJobsList] = useState<ElectricalJobItem[]>([
    {
      ...
      details:
        "4.0 sqmm AC lines, 2.5 sqmm socket circuits & 1.5 sqmm light points with inverter loop.",
    },
    {
      id: "JOB-EL-102",
      ...
      details:
        "Legrand 18-module boards, 63A 4-Pole RCCB & 12-way TPN DB dressing.",
    },
    {
      id: "JOB-EL-103",
      ...
      details:
        "2 x 50mm Copper Pipe gel chemical earthing with test link and 200A busbar cabling.",
    },
    {
      id: "JOB-EL-104",
      ...
      details:
        "Aluminium profile channel cutting with 240 LED/m strip & magnetic track light fitting.",
    },
    {
      id: "JOB-EL-105",
      ...
      details:
        "Emergency night call for 35 sqmm aluminium main cable jointing and 3-phase load audit.",
    },
  ]);
  ...
  const [includeElvAutomation, setIncludeElvAutomation] =
  ...
  const standardRatings = [
    63, 100, 250, 500, 750, 1000, 1250, 1600, 2000, 2500,
  ];
  const targetKva = connectedLoadKVA * 1.25;
  const transformerRatingKVA =
    standardRatings.find((r) => r >= targetKva) || 2500;
  const mainPanelCostINR = Math.round(transformerRatingKVA * 2400 + 180000);
  const wiringConduitCostINR = Math.round(totalAreaSqft * 65);
  const elvAutomationCostINR = includeElvAutomation
    ? Math.round(totalAreaSqft * 38 + 120000)
    : 0;
  const substationCivilCostINR = Math.round(
    transformerRatingKVA * 1800 + 250000,
  );

  const totalEstimatedCostINR =
    mainPanelCostINR +
    wiringConduitCostINR +
    elvAutomationCostINR +
    substationCivilCostINR;
  ...
  const handleAddNewJob = (e: React.FormEvent) => {
    ...
    const newJob: ElectricalJobItem = {
      ...
      details:
        newDetails || "Electrical installation work as specified by client.",
    };
  ...
  const handleUpdateJobStatus = (
    id: string,
    newStatus: ElectricalJobItem["status"],
  ) => {
    setJobsList((prev) =>
  ...
  const totalEarnedINR = jobsList
    .filter((j) => j.status === "Completed" || j.status === "Paid")
    .reduce((sum, j) => sum + j.agreedPriceINR, 0);
  const pendingCollectionINR = jobsList
    .filter((j) => j.status !== "Paid")
    .reduce((sum, j) => sum + (j.agreedPriceINR - j.advancePaidINR), 0);
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/20 text-amber-300 text-xs font-black border border-amber-500/30">
              ...
              <span>
                {isHi
                  ? "इलेक्ट्रीशियन वर्क एवं MEP इलेक्ट्रो-मेकेनिकल Hub"
                  : "Electrician Work & MEP Electro-Mechanical Hub"}
              </span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white">
              {isHi
                ? "इलेक्ट्रीशियन डैशबोर्ड & वर्क ऑटोमेशन स्टूडियो"
                : "Electrician Dashboard & Work Automation Studio"}
            </h1>
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
              {isHi
                ? "वायरिंग (Wiring), फ़िटिंग्स (Fitting), ग्राइंडिंग/अर्थिंग (Earthing/Grinding), डीबी पैनल (DB Panel), फ़ॉल्ट रिपेयर एवं सबस्टेशन लोडिंग का स्मार्ट प्रबंधन।"
                : "Smart management of wiring, fitting, earthing, DB panels, fault repair, and substation load calculations."}
            </p>
          </div>
          ...
          <div className="bg-black/40 p-1.5 rounded-2xl border border-white/20 flex flex-col sm:flex-row gap-2 shrink-0">
            ...
            >
              ...
              <span>
                ⚡{" "}
                {isHi
                  ? "इलेक्ट्रीशियन वर्क डैशबोर्ड (Electrician Work)"
                  : "Electrician Work Dashboard"}
              </span>
            </button>
            ...
            >
              ...
              <span>
                🏭{" "}
                {isHi
                  ? "सबस्टेशन HT/LT लोड कैलकुलेटर"
                  : "Substation HT/LT Load Calculator"}
              </span>
            </button>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-1">
              <span className="text-[11px] font-bold text-slate-500 uppercase block">
                {isHi ? "कुल पूरे किए काम" : "Total Work Orders"}
              </span>
            ...
            <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-1">
              <span className="text-[11px] font-bold text-slate-500 uppercase block">
                {isHi ? "कुल प्राप्त कमाई (Earned)" : "Total Earnings"}
              </span>
              <div className="text-2xl font-black text-emerald-600 dark:text-emerald-400">
                ₹{totalEarnedINR.toLocaleString("en-IN")}
              </div>
              <span className="text-[10px] text-slate-400 font-medium">
                {isHi ? "बैंक/कैश खाते में जमा" : "Collected in Bank/Cash"}
              </span>
            </div>

            <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-1">
              <span className="text-[11px] font-bold text-slate-500 uppercase block">
                {isHi ? "बकाया पेमेंट (Pending)" : "Pending Balance"}
              </span>
              ...
              <span className="text-[10px] text-amber-600 dark:text-amber-300 font-bold">
                {isHi ? "वसूली योग्य बैलेंस" : "To be collected"}
              </span>
            </div>

            <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-1">
              <span className="text-[11px] font-bold text-slate-500 uppercase block">
                {isHi ? "ग्राहक रेटिंग & बैज" : "Client Rating & Badge"}
              </span>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
              <div>
                ...
                <h2 className="text-lg font-black text-slate-900 dark:text-white mt-1 flex items-center gap-2">
                  ...
                  {isHi
                    ? "इलेक्ट्रीशियन वर्क रेट कार्ड एवं तुरंत कोटेशन कैलकुलेटर"
                    : "Electrician Work Rate Card & Quotation Calculator"}
                </h2>
                <p className="text-xs text-slate-500">
                  {isHi
                    ? "प्वाइंट wiring, स्विच बोर्ड, केमिकल अर्थिंग एवं कटिंग के हिसाब से कुल मजदूरी का तुरंत हिसाब लगाएं।"
                    : "Instantly compute labor charges based on point wiring, switch boards, earthing pits, and wall cutting."}
                </p>
              </div>
              ...
              <div className="p-3 bg-amber-50 dark:bg-amber-950/60 rounded-2xl border border-amber-200 dark:border-amber-800 text-right">
                <span className="text-[10px] font-bold text-amber-800 dark:text-amber-300 block">
                  {isHi
                    ? "अनुमानित कुल मजदूरी (Estimated Labor)"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-2">
                <label className="block text-xs font-extrabold text-slate-900 dark:text-white">
                  1.{" "}
                  {isHi
                    ? "लाइट/फ़ैन प्वाइंट्स (Qty)"
                    : "Light/Fan Points (Qty)"}
                </label>
                <input
                  type="number"
                ...
                <div className="flex items-center justify-between text-[10px] text-slate-400">
                  <span>
                    {isHi ? "दर (Rate):" : "Rate:"} ₹{ratePerPoint}/
                    {isHi ? "प्वाइंट" : "pt"}
                  </span>
                  <span className="font-bold text-indigo-600">
                    ₹{(calcPointsCount * ratePerPoint).toLocaleString()}
                  </span>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-2">
                <label className="block text-xs font-extrabold text-slate-900 dark:text-white">
                  2.{" "}
                  {isHi
                    ? "मॉड्युलर स्विच बोर्ड (Qty)"
                    : "Modular Switchboards (Qty)"}
                </label>
                <input
                  type="number"
                ...
                <div className="flex items-center justify-between text-[10px] text-slate-400">
                  <span>
                    {isHi ? "दर (Rate):" : "Rate:"} ₹{ratePerBoard}/
                    {isHi ? "बोर्ड" : "board"}
                  </span>
                  <span className="font-bold text-indigo-600">
                    ₹{(calcBoardsCount * ratePerBoard).toLocaleString()}
                  </span>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-2">
                <label className="block text-xs font-extrabold text-slate-900 dark:text-white">
                  3.{" "}
                  {isHi
                    ? "केमिकल अर्थिंग पिट (Pits)"
                    : "Chemical Earthing Pits (Pits)"}
                </label>
                <input
                  type="number"
                ...
                <div className="flex items-center justify-between text-[10px] text-slate-400">
                  <span>
                    {isHi ? "दर (Rate):" : "Rate:"} ₹{ratePerEarthingPit}/
                    {isHi ? "पिट" : "pit"}
                  </span>
                  <span className="font-bold text-indigo-600">
                    ₹{(calcEarthingPits * ratePerEarthingPit).toLocaleString()}
                  </span>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-2">
                <label className="block text-xs font-extrabold text-slate-900 dark:text-white">
                  4.{" "}
                  {isHi
                    ? "कटिंग/ग्राइंडिंग (Feet)"
                    : "Wall Cutting & Chipping (Feet)"}
                </label>
                <input
                  type="number"
                  value={calcWallCuttingFeet}
                  onChange={(e) =>
                    setCalcWallCuttingFeet(Number(e.target.value))
                  }
                  className="w-full p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-sm"
                />
                <div className="flex items-center justify-between text-[10px] text-slate-400">
                  <span>
                    {isHi ? "दर (Rate):" : "Rate:"} ₹{ratePerCuttingFoot}/
                    {isHi ? "फ़ीट" : "ft"}
                  </span>
                  <span className="font-bold text-indigo-600">
                    ₹
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-2">
                <label className="block text-xs font-extrabold text-slate-900 dark:text-white">
                  5.{" "}
                  {isHi
                    ? "MCB DB बॉक्स ड्रेसिंग (Sets)"
                    : "MCB DB Box Dressing (Sets)"}
                </label>
                <input
                  type="number"
                  value={calcDbPanels}
                ...
                <div className="flex items-center justify-between text-[10px] text-slate-400">
                  <span>
                    {isHi ? "दर (Rate):" : "Rate:"} ₹{ratePerDbPanel}/
                    {isHi ? "सेट" : "set"}
                  </span>
                  <span className="font-bold text-indigo-600">
                    ₹{(calcDbPanels * ratePerDbPanel).toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-xl space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
              <div>
                <h2 className="text-lg font-black text-slate-900 dark:text-white flex items-center gap-2">
                  ...
                  {isHi
                    ? "इलेक्ट्रीशियन कार्य श्रेणियां एवं एक्टिव जॉब्स (Electrical Field Jobs)"
                    : "Electrical Field Jobs & Work Orders"}
                </h2>
                <p className="text-xs text-slate-500">
                  {isHi
                    ? "कार्य के प्रकार के अनुसार फ़िल्टर करें - वायरिंग, फ़िटिंग्स, ग्राइंडिंग/अर्थिंग, डीबी पैनल एवं मेंटेनेंस।"
                    : "Filter by work type - wiring, fittings, earthing, DB panels & maintenance."}
                </p>
              </div>
              ...
              >
                ...
                <span>
                  {isHi
                    ? "नया काम / जॉब दर्ज करें (Add New Job)"
                    : "Add New Job"}
                </span>
              </button>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex flex-wrap items-center gap-2 text-xs font-bold">
                ...
                >
                  🎛️ 2.{" "}
                  {isHi ? "फ़िटिंग्स & बोर्ड (Fitting)" : "Fittings & Boards"}
                </button>
                ...
                >
                  ⚡ 3.{" "}
                  {isHi
                    ? "ग्राइंडिंग, अर्थिंग & DB"
                    : "Grinding, Earthing & DB"}
                </button>
                ...
                >
                  💡 4.{" "}
                  {isHi ? "लाइटिंग फ़िटिंग्स (Lighting)" : "Lighting Fittings"}
                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredJobs.map((job) => (
                <div
                  key={job.id}
                  className="bg-slate-50 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 space-y-4 flex flex-col justify-between"
                >
                  <div className="space-y-3">
                      ...
                      <span
                        className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase ${
                          job.workType === "wiring"
                            ...
                            : job.workType === "fitting"
                              ? "bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-300"
                              : job.workType === "earthing"
                                ? "bg-purple-100 text-purple-800 dark:bg-purple-950 dark:text-purple-300"
                                : job.workType === "lighting"
                    ...
                    <div className="p-2.5 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 text-xs space-y-1">
                      ...
                      <div className="text-[10px] text-slate-400 font-mono">
                        Points/Units: {job.pointsCount} | Scheduled:{" "}
                        {job.scheduledDate}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-3 border-t border-slate-200 dark:border-slate-700 space-y-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <select
                        value={job.status}
                        onChange={(e) =>
                          handleUpdateJobStatus(job.id, e.target.value as any)
                        }
            ...
            <div className="lg:col-span-5 bg-white dark:bg-slate-800/90 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm space-y-6">
              <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-700 pb-3">
                ...
                <h2 className="font-bold text-base text-slate-900 dark:text-white">
                  {isHi
                    ? "सुविधा एवं लोड पैरामीटर्स (Facility & Load Parameters)"
                    : "Facility & Load Parameters"}
                </h2>
              </div>
              ...
              <div className="space-y-4 text-xs">
                <div>
                  ...
                  >
                    <option value="Commercial Complex">
                      {isHi
                        ? "कमर्शियल ऑफिस कॉम्प्लेक्स (8 W/sqft)"
                        : "Commercial Office Complex (8 W/sqft)"}
                    </option>
                    <option value="Industrial Factory">
                      {isHi
                        ? "इंडस्ट्रियल मैन्युफैक्चरिंग फैक्टरी (14 W/sqft)"
                        : "Industrial Manufacturing Factory (14 W/sqft)"}
                    </option>
                    <option value="Residential G+3">
                      {isHi
                        ? "रेजिडेंशियल अपार्टमेंट G+3/G+4 (6 W/sqft)"
                        : "Residential Apartment G+3/G+4 (6 W/sqft)"}
                    </option>
                    <option value="Hospital / Data Center">
                      {isHi
                        ? "अस्पताल / डेटा सेंटर (22 W/sqft)"
                        : "Hospital / Data Center (22 W/sqft High Density)"}
                    </option>
                  </select>
                </div>
                ...
                <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                  <label className="flex items-center gap-2 font-semibold text-slate-700 dark:text-slate-300 cursor-pointer">
                    ...
                    <span>
                      {isHi
                        ? "100% AMF डीजल जनरेटर (DG) बैकअप शामिल करें"
                        : "Include 100% AMF Diesel Generator (DG) Backup"}
                    </span>
                  </label>
                  ...
                  <label className="flex items-center gap-2 font-semibold text-slate-700 dark:text-slate-300 cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        setIncludeElvAutomation(e.target.checked)
                      }
                      className="rounded text-indigo-600 focus:ring-indigo-500"
                    />
                    <span>
                      {isHi
                        ? "ELV इलेक्ट्रॉनिक्स (CCTV, फ़ायर अलार्म & BMS IoT) शामिल करें"
                        : "Include ELV Electronics (CCTV, Fire Alarm & BMS IoT)"}
                    </span>
                  </label>
                </div>
                ...
                <div className="p-3 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 rounded-xl text-[11px] text-amber-900 dark:text-amber-200 space-y-1">
                  <div className="font-bold flex items-center gap-1.5">
                    ...
                    <span>
                      {selectedCity}{" "}
                      {isHi ? "डिस्कॉम ग्रिड नियम:" : "DISCOM Grid Compliance:"}
                    </span>
                  </div>
                  <p>
                    {isHi
                      ? "100 kVA से ऊपर HT 11kV कनेक्शन अनिवार्य है। पेनल्टी से बचने हेतु APFC पैनल आवश्यक है।"
                      : "HT 11kV connection mandatory above 100 kVA load. APFC panel required to maintain 0.99 PF & prevent penalty surcharges."}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-slate-800/90 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs">
                  <div className="text-[11px] font-semibold text-slate-500">
                    {isHi ? "कनेक्टेड डिमांड" : "Connected Demand"}
                  </div>
                  ...
                  <div className="text-[10px] text-slate-400 mt-0.5">
                    {connectedLoadKW} kW Peak Power
                  </div>
                </div>

                <div className="bg-white dark:bg-slate-800/90 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs">
                  <div className="text-[11px] font-semibold text-slate-500">
                    {isHi ? "सबस्टेशन रेटिंग" : "Substation Rating"}
                  </div>
                  <div className="text-xl font-black text-amber-600 dark:text-amber-400 mt-1">
                    {transformerRatingKVA}{" "}
                    <span className="text-xs font-semibold">kVA</span>
                  </div>
                  <div className="text-[10px] text-slate-400 mt-0.5">
                    HT/LT Transformer
                  </div>
                ...
                <div className="bg-white dark:bg-slate-800/90 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs col-span-2 sm:col-span-1">
                  <div className="text-[11px] font-semibold text-slate-500">
                    {isHi ? "टर्नकी MEP अनुमान" : "Turnkey MEP Estimate"}
                  </div>
                  <div className="text-xl font-black text-indigo-600 dark:text-indigo-400 mt-1">
                    ₹{(totalEstimatedCostINR / 100000).toFixed(2)}{" "}
              ...
              <div className="bg-slate-900 text-white p-5 rounded-2xl flex flex-col sm:flex-row justify-between items-center gap-4">
                <div>
                  <div className="font-bold text-sm">
                    {isHi
                      ? "क्लास-A इलेक्ट्रीशियन ठेकेदार की आवश्यकता है?"
                      : "Need a Class-A Electrical Contractor?"}
                  </div>
                  <p className="text-xs text-slate-400">
                    {isHi
                      ? "लाइसेंस प्राप्त MEP वेंडरों को टेंडर हेतु आमंत्रित करें एवं डिजिटल अनुबंध करें।"
                      : "Invite licensed MEP vendors for competitive tenders and sign digitally bound contracts."}
                  </p>
                </div>
                <button
                  onClick={() =>
                    onNavigateToVendors &&
                    onNavigateToVendors("Electrical & ELV")
                  }
                ...
                >
                  {isHi
                    ? "MEP वेंडर टेंडर जारी करें"
                    : "Issue MEP Vendor Tender"}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3">
              <h3 className="font-black text-sm text-slate-900 dark:text-white flex items-center gap-2">
                ...
                {isHi
                  ? "नया इलेक्ट्रीशियन वर्क / जॉब जोड़ें"
                  : "Add New Electrical Work Order / Job"}
              </h3>
              <button
            ...
            <form onSubmit={handleAddNewJob} className="space-y-3 text-xs">
              <div>
                <label className="block font-bold mb-1">
                  {isHi ? "ग्राहक / क्लाइंट नाम *" : "Client / Customer Name *"}
                </label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <label className="block font-bold mb-1">
                    {isHi ? "स्थान / ज़िला" : "Location / City"}
                  </label>
                  <input
                    type="text"
                    value={newLocation}
              ...
              <div>
                <label className="block font-bold mb-1">
                  {isHi ? "काम की श्रेणी (Work Category) *" : "Work Category *"}
                </label>
                <select
                  value={newWorkType}
                ...
                >
                  ...
                  <option value="fitting">
                    🎛️ Fitting (
                    {isHi
                      ? "स्विच बोर्ड & MCB फ़िटिंग्स"
                      : "Switchboard & MCB Fittings"}
                    )
                  </option>
                  <option value="earthing">
                    ...
                    {isHi
                      ? "केमिकल अर्थिंग & पैनल"
                      : "Chemical Earthing & Panel"}
                    )
                  </option>
                  <option value="lighting">
                    💡 Lighting (
                    {isHi
                      ? "एलईडी प्रोफ़ाइल & फ़ैंसी लाइटिंग"
                      : "LED Profile & Decorative Lighting"}
                    )
                  </option>
                  <option value="maintenance">
                    🛠️ Maintenance (
                    {isHi
                      ? "फ़ॉल्ट रिपेयर & नाइटकॉल"
                      : "Fault Repair & Breakdown"}
                    )
                  </option>
              ...
              <div>
                <label className="block font-bold mb-1">
                  {isHi ? "काम का विवरण (Work Title) *" : "Work Title *"}
                </label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className="block font-bold mb-1">
                    {isHi ? "कुल प्वाइंट्स" : "Total Points"}
                  </label>
                  <input
                    type="number"
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={
                    isHi
                      ? "तार की मोटाई, स्विच ब्रांड एवं विशेष निर्देश..."
                      : "Wire gauge, switch brand, special instructions..."
                  }
                  className="w-full p-2 bg-slate-50 dark:bg-slate-800 border rounded-xl"
                />
              </div>
