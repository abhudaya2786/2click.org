...
export const PlatformFeesPublicCard: React.FC<PlatformFeesPublicCardProps> = ({
  systemSettings,
  compact = false,
}) => {
  const [selectedDuration, setSelectedDuration] = useState<
    "1_month" | "1_year" | "3_year"
  >("1_year");
  ...
  const fees = systemSettings?.platformFees || {
    ...
    notes:
      "Official platform registration, verification badge & escrow protected deal access.",
  };

  // Duration multiplier and discount calculation
  const getDurationData = (baseYearlyFee: number) => {
    if (baseYearlyFee === 0)
      return { fee: 0, durationLabel: "Lifetime FREE", savings: "100% Free" };
    if (selectedDuration === "1_month") {
      const monthlyFee = Math.round((baseYearlyFee * 1.25) / 12);
      return {
        fee: monthlyFee,
        durationLabel: "प्रति माह (Per Month)",
        savings: "Monthly Pay",
      };
    }
    if (selectedDuration === "1_year") {
      return {
        fee: baseYearlyFee,
        durationLabel: "1 वर्ष (1 Year Access)",
    ...
    return {
      fee: threeYearFee,
      durationLabel: "3 वर्ष (3 Years VIP Access)",
      savings: "30% Mega Discount",
    };
  };
  ...
  const membershipPlans = [
    {
      ...
      badge: "Free Member",
      features: [
        "2D/3D AI Naksha maps",
        "Direct Vendor Bidding",
    ...
    },
    {
      ...
      features: [
        "Local Buyer Lead Alerts",
        "Digital Rate Board Listing",
    ...
    },
    {
      ...
      features: [
        "Direct Construction Tenders",
        "Unlimited Material Listing",
    ...
    },
    {
      ...
      features: [
        "DISCOM Net-metering Portal",
        "Govt Subsidy Help Desk",
        "District Direct Franchise",
      ],
    },
    {
      ...
      badge: "Material Hub",
      features: [
        "Bulk Mill Direct Orders",
        "Transporter Freight Match",
        "Verified Rate Catalog",
      ],
    },
  ];
  ...
  const serviceChargeItems = [
    {
      serviceName:
        "सोलर पैनल व इनवर्टर इंस्टॉलेशन (Solar System Rooftop Installation)",
    ...
    {
      serviceName:
        "आर्किटेक्ट AI नक्शा व वास्तु मैप (Architect 2D/3D Floor Plan)",
    ...
    {
      serviceName:
        "सिविल इंजीनियर लोड ऑडिट (Structural Civil Load Audit & BOQ)",
    ...
    {
      serviceName:
        "इलेक्ट्रिशियन व प्लंबर ऑन-डिमांड सर्विस (Electrician & Plumber Visit)",
    ...
    {
      serviceName:
        "मटीरियल क्रेन व ट्रक होम डिलीवरी (Crane Freight Transport Delivery)",
      ...
      <div className="p-4 bg-gradient-to-r from-slate-900 via-teal-950 to-indigo-950 text-white rounded-2xl border border-teal-500/30 space-y-3 shadow-xl">
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
          <span className="font-extrabold flex items-center gap-1.5 text-teal-300">
            <Tag className="w-4 h-4 text-amber-400" /> मेंबरशिप फ़ीस, सर्विस
            चार्ज एवं टाइम समयावधि (Platform Rates)
          </span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-[11px]">
          {membershipPlans.map((plan, idx) => {
            ...
            return (
              <div
                key={idx}
                className="p-2.5 bg-white/5 rounded-xl border border-white/10 text-center flex flex-col justify-between"
              >
                ...
                <div className="text-[9px] text-teal-300 font-semibold">
                  {data.durationLabel}
                </div>
              </div>
            );
    ...
    <div className="p-6 bg-gradient-to-br from-slate-900 via-teal-950 to-indigo-950 text-white rounded-3xl border border-teal-500/40 shadow-2xl space-y-6 my-6 relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-teal-500/30 pb-5">
        <div className="space-y-1">
          <span className="px-3 py-1 bg-teal-500/20 text-teal-300 border border-teal-500/40 rounded-lg text-xs font-black uppercase tracking-wider flex items-center gap-1.5 w-fit">
            <Sparkles className="w-3.5 h-3.5 text-amber-400" /> Official
            Membership Fees &amp; Service SLA Time Schedule
          </span>
          <h3 className="text-xl sm:text-2xl font-black text-white">
            मेंबरशिप शुल्क, सर्विस चार्ज एवं कार्य समयावधि (Platform Rates &amp;
            Durations)
          </h3>
          <p className="text-xs text-slate-300">
            2CLICK प्लेटफ़ॉर्म पारदर्शी मूल्य निर्धारण: रोल अनुसार मेंबरशिप
            प्लान्स तथा प्रत्येक सर्विस का सर्विस चार्ज व गारंटी टाइम अवधि
            देखें।
          </p>
        </div>
          ...
          <div className="p-4 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="grid grid-cols-3 gap-2 w-full sm:w-auto">
              ...
              >
                ...
                <span className="text-[9px] text-emerald-950 font-black uppercase">
                  30% छूट Save
                </span>
              </button>
            </div>
                ...
                >
                  ...
                  <div className="p-3 bg-black/40 rounded-xl border border-white/10 space-y-1">
                    <div className="text-[9px] text-slate-400 font-extrabold uppercase">
                      मेंबरशिप शुल्क (Membership Rate)
                    </div>
      ...
      {feeTab === "services" && (
        <div className="space-y-4 animate-in fade-in duration-200">
          <div className="p-4 bg-teal-900/30 rounded-2xl border border-teal-500/30 text-xs text-teal-200 flex items-center gap-2">
            ...
            <div>
              <strong>
                प्रत्येक सेवा का निर्धारित सर्विस चार्ज एवं कार्य पूरा होने की
                समयावधि:
              </strong>{" "}
              नीचे दी गई सभी सेवाएं लाइसेंस प्राप्त पेशेवरों द्वारा एस्क्रो
              सुरक्षा के साथ पूरी की जाती हैं।
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {serviceChargeItems.map((item, idx) => (
              <div
                key={idx}
                className="p-4 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 space-y-3 hover:border-amber-400/50 transition"
              >
      ...
      <div className="p-3 bg-black/40 rounded-xl border border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-300">
        <div className="flex items-center gap-2">
          ...
          <span>
            {fees.notes ||
              "100% Verified Escrow Protected & GST Compliant Services"}
          </span>
        </div>
        {fees.gstApplicable && (
