...
export const PublicUserShowcaseModal: React.FC<
  PublicUserShowcaseModalProps
> = ({ user, isOpen, onClose, isPreviewMode = false }) => {
  ...
  const products =
    user.customProducts && user.customProducts.length > 0
      ? user.customProducts
      : [
          {
            id: "PROD-1",
            ...
            inStock: true,
            description:
              "High-strength Portland Pozzolana Cement for slab casting & masonry.",
          },
          {
            id: "PROD-2",
            ...
            inStock: true,
            description:
              "Ductile high-yield TMT rebar with earthquake resistant properties.",
          },
          {
            id: "PROD-3",
            ...
            unit: "Set",
            imageUrl:
              "https://images.unsplash.com/photo-1508873696983-2df5057d0256?auto=format&fit=crop&w=600&q=80",
            inStock: true,
            description:
              "Tier-1 Bifacial panels with 5kVA Solar Inverter & Net Metering guidance.",
          },
        ];
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-4xl rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden my-8 my-auto relative animate-in fade-in zoom-in duration-200">
        ...
        {isPreviewMode && (
          <div className="bg-amber-500 text-slate-950 px-4 py-2 font-black text-xs text-center flex items-center justify-center gap-2">
            ...
            <span>
              👁️ PUBLIC PREVIEW MODE: यह रूप आपके ग्राहक / क्लाइंट को लिंक खोलने
              पर दिखेगा
            </span>
          </div>
        )}
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
            ...
            <div className="space-y-1.5 flex-1">
              ...
              <p className="text-slate-300 text-xs sm:text-sm font-medium">
                {settings.headlineMessage ||
                  `Certified ${user.role} in ${user.district || user.city || "India"}`}
              </p>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mt-6 pt-6 border-t border-slate-800">
            ...
            {settings.showGstin && user.gstinNumber && (
              <span className="text-[11px] text-slate-400 font-bold bg-black/30 px-3 py-1 rounded-lg">
                GSTIN:{" "}
                <strong className="text-teal-300">{user.gstinNumber}</strong>
              </span>
            )}
          </div>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {products.map((p) => (
                  <div
                    key={p.id}
                    className="bg-slate-50 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 space-y-3 flex flex-col justify-between hover:shadow-lg transition"
                  >
            ...
            <div className="bg-gradient-to-r from-teal-900/10 via-emerald-900/10 to-indigo-900/10 p-5 rounded-3xl border border-teal-500/20 flex flex-col sm:flex-row items-center justify-between gap-6">
              <div className="space-y-1 text-center sm:text-left">
                ...
                <p className="text-xs text-slate-500">
                  Scan QR code or use official UPI handle for instant billing
                  settlement.
                </p>
              </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 p-5 rounded-3xl border border-slate-200 dark:border-slate-700 space-y-3">
            ...
            {inquirySent ? (
              <div className="p-4 bg-emerald-100 text-emerald-900 rounded-2xl text-xs font-bold flex items-center gap-2">
                ...
                <span>
                  आपका मैसेज सफलतापूर्वक भेज दिया गया है! विक्रेता जल्द ही
                  संपर्क करेगा।
                </span>
              </div>
            ) : (
