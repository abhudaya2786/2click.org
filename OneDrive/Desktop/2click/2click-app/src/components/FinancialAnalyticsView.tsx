...
const PROJECT_BUDGET_DATA = [
  {
    projectName: "Gorakhpur Commercial Complex",
    allocatedBudget: 280.0,
    actualSpent: 242.5,
    committedCost: 25.0,
    variance: +12.5,
    status: "On Track",
  ...
  {
    projectName: "Varanasi Luxury Villa Fitout",
    allocatedBudget: 95.0,
    actualSpent: 92.8,
    committedCost: 4.2,
    variance: -2.0,
    status: "Near Limit",
  ...
  {
    projectName: "Kanpur Industrial Warehouse ETP",
    allocatedBudget: 180.0,
    actualSpent: 165.0,
    committedCost: 12.0,
    variance: +3.0,
    status: "On Track",
  ...
  {
    projectName: "Prayagraj Highway Smart Lighting",
    allocatedBudget: 110.0,
    actualSpent: 116.5,
    committedCost: 8.0,
    variance: -14.5,
    status: "Over Budget",
  ...
  {
    projectName: "Ayodhya Hotel Heritage Interior",
    allocatedBudget: 210.0,
    actualSpent: 185.2,
    committedCost: 18.0,
    variance: +6.8,
    status: "On Track",
...
const VENDOR_PAYMENT_LEDGER = [
  {
    id: "INV-2026-881",
    vendorName: "Tata Tiscon Steel Distributors",
    category: "Materials",
    project: "Gorakhpur Commercial Complex",
    amountINR: 1850000,
  ...
  {
    id: "INV-2026-882",
    vendorName: "Kajaria World Tiles & Marble",
    category: "Interiors",
    project: "Ayodhya Hotel Heritage Interior",
    amountINR: 1240000,
  ...
  {
    id: "INV-2026-883",
    vendorName: "Waaree Solar Modules Pvt Ltd",
    category: "Solar",
    project: "Lucknow Metro Solar Rooftop 500kW",
    amountINR: 2450000,
    invoiceDate: "2026-07-25",
    dueDate: "2026-08-15",
    status: "Due within 15 Days",
    paymentMode: "2click Escrow",
  },
  {
    id: "INV-2026-884",
    vendorName: "UltraTech Cement Bulk Depot",
    category: "Materials",
    project: "Kanpur Industrial Warehouse ETP",
    amountINR: 980000,
  ...
  {
    id: "INV-2026-885",
    vendorName: "Blum Austrian Hardware India",
    category: "Interiors",
    project: "Varanasi Luxury Villa Fitout",
    amountINR: 650000,
  ...
  {
    id: "INV-2026-886",
    vendorName: "Havells Heavy Electricals",
    category: "Electrical",
    project: "Prayagraj Highway Smart Lighting",
    amountINR: 1420000,
    invoiceDate: "2026-07-28",
    dueDate: "2026-08-20",
    status: "Under GST Audit",
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-900/95 border border-slate-700 p-3 rounded-xl shadow-xl text-xs text-slate-100 backdrop-blur-md">
        <p className="font-bold border-b border-slate-700 pb-1mb-2 text-teal-400">
          {label}
        </p>
...
const CustomTooltipPie = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-slate-900/95 border border-slate-700 p-3 rounded-xl shadow-xl text-xs text-slate-100 backdrop-blur-md">
        <p
          className="font-bold border-b border-slate-700 pb-1 mb-1"
        ...
        <div className="space-y-1 font-medium">
          <div className="flex justify-between gap-4">
            ...
            <span className="font-bold text-white">
              {data.payload.count} Vendors
            </span>
          </div>
          <div className="flex justify-between gap-4">
            <span className="text-slate-400">Total Volume:</span>
            <span className="font-mono font-bold text-teal-300">
              ₹{data.payload.valueLakhs.toFixed(1)} Lakhs
            </span>
          </div>
        </div>
      </div>
...
export const FinancialAnalyticsView: React.FC<FinancialAnalyticsViewProps> = ({
  ...
  const [expenseChartType, setExpenseChartType] = useState<
    "stacked" | "bar" | "composed"
  >("stacked");
  const [vendorFilter, setVendorFilter] = useState<string>("All");
  const [searchLedger, setSearchLedger] = useState<string>("");

  const displayExpenseData =
    timeRange === "6m" ? MONTHLY_EXPENSE_DATA.slice(-6) : MONTHLY_EXPENSE_DATA;

  // Calculate Aggregates
  const totalExpensesYTD = displayExpenseData.reduce(
    (acc, curr) => acc + curr.totalExpense,
    0,
  );
  const totalBudgetYTD = displayExpenseData.reduce(
    (acc, curr) => acc + curr.targetBudget,
    0,
  );
  const totalAllocatedProjects = PROJECT_BUDGET_DATA.reduce(
    (acc, curr) => acc + curr.allocatedBudget,
    0,
  );
  const totalSpentProjects = PROJECT_BUDGET_DATA.reduce(
    (acc, curr) => acc + curr.actualSpent,
    0,
  );
  const totalVendorPending =
    VENDOR_PAYMENT_STATUS_DATA.find((s) => s.status === "Pending Approval")
      ?.valueLakhs || 0;
  const totalVendorDue15 =
    VENDOR_PAYMENT_STATUS_DATA.find((s) => s.status === "Due within 15 Days")
      ?.valueLakhs || 0;
  const totalVendorOverdue =
    VENDOR_PAYMENT_STATUS_DATA.find((s) => s.status === "Overdue / Delayed")
      ?.valueLakhs || 0;

  const filteredLedger = VENDOR_PAYMENT_LEDGER.filter((item) => {
    const matchesFilter =
      vendorFilter === "All" || item.status === vendorFilter;
    const matchesSearch =
      item.vendorName.toLowerCase().includes(searchLedger.toLowerCase()) ||
    ...
    <div className="space-y-8 animate-fade-in">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2">
            ...
            <p className="text-xs sm:text-sm text-slate-300 max-w-2xl leading-relaxed">
              Real-time cash flow monitoring, monthly expense breakdowns across
              engineering verticals, project budget utilization, and vendor
              payment audit ledgers.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            ...
            <button
              onClick={() =>
                alert(
                  "Downloading Financial Analytics Statement (PDF / Excel)...",
                )
              }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between space-y-3">
          ...
          <div>
            ...
            <div className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 flex items-center gap-1 mt-1">
              ...
              <span>
                3.2% under allocated budget (₹
                {(totalBudgetYTD / 100).toFixed(2)} Cr)
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between space-y-3">
          ...
          <div>
            <div className="text-2xl font-black text-slate-900 dark:text-white">
              ₹{(totalSpentProjects / 100).toFixed(2)} Cr{" "}
              <span className="text-xs font-normal text-slate-400">
                / ₹{(totalAllocatedProjects / 100).toFixed(2)} Cr
              </span>
            </div>
            <div className="text-xs font-semibold text-teal-600 dark:text-teal-400 flex items-center gap-1 mt-1">
              <TrendingUp className="w-3.5 h-3.5" />
              <span>
                {((totalSpentProjects / totalAllocatedProjects) * 100).toFixed(
                  1,
                )}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Civil materials, labor wages, equipment logistics, solar kits
              &amp; interior fittings in ₹ Lakhs.
            </p>
          </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            {expenseChartType === "stacked" ? (
              ...
              >
                ...
                <Area
                  type="monotone"
                  dataKey="interiorFitout"
                  name="Interior Fitouts"
                  stackId="1"
                  stroke="#6366f1"
                  fill="url(#gradInterior)"
                />
                <Area
                  type="monotone"
                  dataKey="solarEquipment"
                  name="Solar Rooftops"
                  stackId="1"
                  stroke="#f59e0b"
                  fill="url(#gradSolar)"
              ...
              >
                ...
                />
                <Bar
                  dataKey="equipmentLogistics"
                  name="Logistics & Machinery"
                  fill="#06b6d4"
                  radius={[4, 4, 0, 0]}
                />
              </BarChart>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-md space-y-5">
          <div className="flex justify-between items-center pb-3 border-b border-slate-100 dark:border-slate-800">
            <div>
              <h3 className="font-black text-base text-slate-900 dark:text-white flex items-center gap-2">
                ...
                Active Project Budgets vs Actual Spend (प्रोजेक्ट बजट एवं
                वास्तविक व्यय)
              </h3>
              <p className="text-xs text-slate-500">
                Allocated budget vs actual spent + committed costs in ₹ Lakhs
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
            {PROJECT_BUDGET_DATA.slice(0, 3).map((p, idx) => (
              <div
                key={idx}
                className="p-3 bg-slate-50 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-1"
              >
                ...
                <div className="w-full bg-slate-200 dark:bg-slate-700 h-1.5 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (p.actualSpent / p.allocatedBudget) * 100)}%`,
                    }}
                  />
                </div>
              </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-md space-y-5 flex flex-col justify-between">
          <div className="pb-3 border-b border-slate-100 dark:border-slate-800">
            ...
            <p className="text-xs text-slate-500">
              Breakdown of invoices by audit &amp; release state
            </p>
          </div>
          ...
          <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
            {VENDOR_PAYMENT_STATUS_DATA.map((st, i) => (
              <div
                key={i}
                className="flex justify-between items-center text-slate-700 dark:text-slate-300 font-medium"
              >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-md space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Filter by invoice status, track due dates, GST compliance &amp;
              payment release modes
            </p>
          </div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        className={`px-2.5 py-1 rounded-lg text-[10px] font-black flex items-center gap-1 ${
                          row.status === "Paid in Full"
                            ...
                            : row.status === "Pending Approval"
                              ...
                              : row.status === "Due within 15 Days"
                                ? "bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300"
                                : row.status === "Overdue / Delayed"
                                  ? "bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300"
                      ...
                      {row.status !== "Paid in Full" && (
                        <button
                          onClick={() =>
                            alert(
                              `Initiating payment release for ${row.id} (${row.vendorName}) via 2click Escrow...`,
                            )
                          }
