...
export const AiCopilotDrawer: React.FC<AiCopilotDrawerProps> = ({
  ...
  const [generatedImages, setGeneratedImages] = useState<
    Array<{ url: string; prompt: string; ratio: string; size: string }>
  >([]);
  ...
  const quickPrompts = [
    "📍 Find top TMT Steel and Cement suppliers near me",
    "🎨 Generate 3D Elevation render for 30x50 ft modern house",
    "☀️ Calculate PM Surya Ghar solar subsidy for 3kW plant",
    "📐 Estimate RCC slab materials for 1200 sqft house",
    "⚖️ What are late filing GST rules for contractors?",
  ];
  ...
  const sendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      const aiMsg: ChatMessage = {
        ...
        text:
          data.reply ||
          "Apologies, I encountered an error processing your query.",
        timestamp: new Date().toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        }),
        sources: data.groundingChunks
          ? data.groundingChunks.map(
              (c: any) => c.web?.title || "Google Maps Grounding",
            )
          : ["IS 456 Code", "CPWD DSR"],
      };
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-violet-200 font-medium">
              Maps Grounding · Voice Transcription · 3D Renders
            </p>
          </div>
        </div>
      ...
      {mode === "image" && (
        <div className="p-3 bg-violet-50 dark:bg-violet-950/30 border-b border-violet-200 dark:border-violet-800/50 space-y-2 text-xs">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] text-slate-500 dark:text-slate-400 font-medium mb-1">
                Aspect Ratio
              </label>
              <select
                value={aspectRatio}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="space-y-3 mb-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              {generatedImages.map((img, i) => (
                <div
                  key={i}
                  className="rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-2 space-y-2"
                >
                  <img
                    src={img.url}
                    alt={img.prompt}
              ...
              {m.sources && m.sources.length > 0 && (
                <div className="mt-2 pt-2 border-t border-slate-200/40 dark:border-slate-700/60 flex flex-wrap gap-1 text-[9px]">
                  ...
                  {m.sources.map((s, idx) => (
                    <span
                      key={idx}
                      className="px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-700 font-mono"
                    >
                      {s}
                    </span>
                  ))}
        ...
        {loading && (
          <div className="flex items-center gap-2 p-3 bg-slate-100 dark:bg-slate-800 rounded-2xl w-fit text-xs text-slate-500">
            ...
            <span>
              {mode === "image"
                ? `Generating ${imageSize} 3D Render (${aspectRatio})...`
                : "Consulting Gemini AI & Local Grounding..."}
            </span>
          </div>
        )}
        ...
        <div className="flex gap-1.5 whitespace-nowrap overflow-x-auto pb-1">
          {quickPrompts.map((qp, idx) => (
            <button
              ...
              className="px-2.5 py-1 bg-white dark:bg-slate-800 hover:bg-teal-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-[10px] font-medium border border-slate-200 dark:border-slate-700 transition"
            >
              {qp}
            </button>
          ))}
      ...
      <div className="p-3 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
        ...
        >
          ...
          <input
            ...
            placeholder={
              mode === "image"
                ? "Describe 3D elevation or interior render..."
                : "Ask AI Copilot or speak into mic..."
            }
