...
const PIN_DICTIONARY: Record<
  string,
  { city: string; subArea: string; baseLat: number; baseLng: number }
> = {
  "273212": {
    city: "Gorakhpur",
    subArea: "Campierganj",
  ...
  "273001": {
    city: "Gorakhpur",
    subArea: "Gorakhpur City",
    baseLat: 26.7606,
    baseLng: 83.3732,
  },
  "273015": {
    city: "Gorakhpur",
    subArea: "GDA Colony & AIIMS",
    baseLat: 26.721,
    baseLng: 83.411,
  },
  "272001": {
  ...
  "560001": {
    city: "Bengaluru",
    subArea: "MG Road / Central",
    baseLat: 12.9716,
    baseLng: 77.5946,
  },
  "560066": {
    city: "Bengaluru",
    subArea: "Whitefield Tech Belt",
    baseLat: 12.9698,
    baseLng: 77.75,
  },
  "400001": {
    city: "Mumbai",
    subArea: "Fort / South Mumbai",
    baseLat: 18.9322,
    baseLng: 72.8347,
  },
  "110001": {
  ...
  "600001": {
    city: "Chennai",
    subArea: "Parrys / Central",
    baseLat: 13.0827,
    baseLng: 80.2707,
  },
  "411001": {
    city: "Pune",
    subArea: "Pune Station / Camp",
    baseLat: 18.5204,
    baseLng: 73.8567,
  },
  "500001": {
    city: "Hyderabad",
    subArea: "Abids / Secunderabad",
    baseLat: 17.385,
    baseLng: 78.4867,
  },
  "226001": {
  ...
  "302001": {
    city: "Jaipur",
    subArea: "Pink City Central",
    baseLat: 26.9124,
    baseLng: 75.7873,
  },
  "700001": {
    city: "Kolkata",
    subArea: "BBD Bagh Central",
    baseLat: 22.5726,
    baseLng: 88.3639,
  },
  "380001": {
...
export const LocationProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [scopeMode, setScopeMode] = useState<LocationScopeMode>(() => {
    return (
      (localStorage.getItem("2click_scope_mode") as LocationScopeMode) ||
      "radius"
    );
  });

  const [pinCode, setPinCodeState] = useState<string>(() => {
    return localStorage.getItem("2click_pincode") || "273212";
  });
  ...
  const [selectedCity, setSelectedCityState] = useState<string>(() => {
    return localStorage.getItem("2click_selected_city") || "Gorakhpur";
  });
  ...
  const getDistanceInfo = (
    ...
    if (scopeMode === "global") {
      return {
        ...
        badgeLabel: isGlobalVendor
          ? "🌐 Global Imported Brand"
          : "🌐 NRI / Global Scope",
    ...
    if (itemPin && PIN_DICTIONARY[itemPin] && PIN_DICTIONARY[pinCode]) {
      ...
      if (itemPin === pinCode) {
        ...
        distance =
          Math.round((Math.abs(src.baseLat - dest.baseLat) * 110 + 4.2) * 10) /
          10;
        targetArea = dest.subArea;
      } else {
        distance =
          Math.round(
            (Math.abs(src.baseLat - dest.baseLat) * 110 +
              Math.abs(src.baseLng - dest.baseLng) * 85 +
              15) *
              10,
          ) / 10;
      ...
      const hash = (itemPin || itemCity || "Local")
        .split("")
        .reduce((acc, char) => acc + char.charCodeAt(0), 0);
      const isSameCity =
        (itemCity || "").toLowerCase() === selectedCity.toLowerCase();
