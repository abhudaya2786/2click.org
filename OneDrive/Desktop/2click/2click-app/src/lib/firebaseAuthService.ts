...
export async function syncUserProfileInFirestore(
  fbUser: FirebaseUser,
  defaultProps?: Partial<User>,
): Promise<User> {
  const userRef = doc(db, "users", fbUser.uid);
  const isSuperAdminEmail =
    fbUser.email && SUPER_ADMIN_EMAILS.includes(fbUser.email.toLowerCase());

  try {
    const snap = await getDoc(userRef);
    if (snap.exists()) {
      const data = snap.data();
      const updatedRole = isSuperAdminEmail
        ? "SuperAdmin"
        : data.role || defaultProps?.role || "Client";
      ...
      const userObj: User = {
        ...
        companyName:
          ...
          `${updatedRole} Enterprise`,
        status: data.status || "Active",
        isKycVerified: data.isKycVerified ?? true,
        employeeCode:
          data.employeeCode ||
          (updatedRole.includes("Employee") || updatedRole.includes("Admin")
      ...
      const newUserObj: User = {
        ...
        isKycVerified: true,
        employeeCode:
          initialRole.includes("Employee") || initialRole.includes("Admin")
            ? `EMP-${Math.floor(1000 + Math.random() * 9000)}`
            : undefined,
      };
