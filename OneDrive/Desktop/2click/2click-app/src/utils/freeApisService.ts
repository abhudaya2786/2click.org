...
export async function getFreeReverseGeocode(
  lat: number,
  lon: number,
  ...
  try {
    ...
    console.warn(
      "Free Nominatim Geocoding error, falling back to IP location:",
      err,
    );
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      resolve({
        ...
        gateway:
          channel === "whatsapp"
            ? "2CLICK WhatsApp Direct API (Free)"
            : "2CLICK Free SMS Gateway",
      });
    }, 400);
  });
}
...
export const LIFETIME_FREE_APIS_INFO = [
  {
    ...
    description:
      "Instant WhatsApp & SMS OTP generation with auto-fill test code (123456) & instant toast alert.",
    features: [
      "WhatsApp Direct OTP",
      "SMS OTP Simulation",
      "Auto-fill Demo Code",
  ...
  },
  {
    ...
    description:
      "Auto-detects user district, city, state, and pincode via GPS coordinates or IP address.",
    features: [
      "GPS Accuracy",
      "IP Address Fallback",
      "Reverse Geocoding",
  ...
  },
  {
    ...
    description:
      "Provides real-time temperature, solar irradiance (GHI/DNI), and wind speed for solar & civil sites.",
    features: [
      "Solar Panel Yield",
      "Temperature Track",
      "No Key Required",
  ...
  },
  {
    ...
    description:
      "Calculates per-kilometer freight rates for Tippers, Tata Ace, and Trailers across 750+ Indian districts.",
    features: [
      "Ton-Km Rate Engine",
